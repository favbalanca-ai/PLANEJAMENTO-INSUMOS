...
const APP_VERSION = '2026.07.18-34';   // mostrado no rodapé; ajude a confirmar se a atualização chegou
...
V.precos = function(){
  ...
  const refRows=s.refs.map((r,i)=>`<tr>
      <td><input class="txt" data-pr="refClasse" data-i="${i}" value="${esc(r.classe||'')}" placeholder="classe"></td>
      <td><input class="txt" data-pr="refProd" data-i="${i}" value="${esc(r.produto||'')}" placeholder="produto de referência"></td>
  ...
  <div class="panel"><div class="panel-head"><h2>Alimentar planejamento</h2><span class="sub">grava os preços desta safra no PORTIFÓLIO do planejamento</span></div>
    <div class="bulk-add" style="align-items:center">
      ${temUrl
        ? `<button class="btn btn-primary btn-sm" data-act="prAlimVista">▶ Alimentar (à vista)</button>
           <button class="btn btn-outline btn-sm" data-act="prAlimPrazo">▶ Alimentar (a prazo)</button>
           <span class="mut" style="font-size:12px">Casa pelo nome do produto e mostra um resumo antes de gravar (atualizados · sem preço nesta safra · novos).</span>`
        : `<span class="mut" style="font-size:12px">Configure a URL na tela <b>Sincronizar</b> (módulo Planejamento) para alimentar o planejamento.</span>`}
    </div>
  </div>
