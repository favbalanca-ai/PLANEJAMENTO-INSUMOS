...
const APP_VERSION = '2026.07.28-74';   // mostrado no rodapé; ajude a confirmar se a atualização chegou
...
V.dashboard = function(){
  ...
  ${(()=>{ const aFazer=compras.filter(r=>r.comprar>0).sort((a,b)=>b.valor-a.valor);
    ...
    <div class="table-wrap"><table><thead><tr><th>Produto</th><th>Classe</th><th class="num">Demanda</th><th class="num">Estoque</th><th class="num">Em pedido</th><th class="num">Comprar</th><th>Un</th><th class="num">Valor</th></tr></thead>
    <tbody>${aFazer.slice(0,30).map(r=>`
      <tr><td><b>${esc(r.produto)}</b></td><td><span class="classe-tag">${esc(r.classe)}</span></td>
      <td class="num">${num(r.demanda)}</td><td class="num">${num(r.estoque||0)}</td><td class="num">${num(r.pedido||0)}</td>
      <td class="num"><b>${num(r.comprar)}</b></td><td>${esc(r.un)}</td><td class="num">${r.preco>0?brl(r.valor):'<span style="color:var(--red)">s/ preço</span>'}</td></tr>`).join('')||'<tr><td colspan="8" class="empty" style="padding:16px">Nada a comprar 🎉</td></tr>'}</tbody></table></div>
...
V.estoque=function(){
  ...
  const rows=[...nomes].map(n=>{ const p=PROD[n]||{produto:n,classe:'',un:''};
    ...
    return {produto:n, classe:p.classe||'', un:p.un||'', est, sai, saldo:est-sai, ped, preco}; })
  ...
  const body=rows.map(r=>{
    ...
    return `<tr data-search="${esc((r.produto+' '+r.classe).toLowerCase())}">
      <td class="c-full" data-th="Produto"><b>${esc(r.produto)}</b>${r.classe?` <span class="classe-tag">${esc(r.classe)}</span>`:''}</td>
...
function filterEstoque(){
  ...
  document.querySelectorAll('#est-tbl tbody tr').forEach(tr=>{
    const s=tr.getAttribute('data-search')||''; tr.style.display=(!q||s.includes(q))?'':'none'; });
}
