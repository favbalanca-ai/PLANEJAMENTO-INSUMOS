...
const APP_VERSION = '2026.07.28-127';   // mostrado no rodapé; ajude a confirmar se a atualização chegou
...
V.dre = function(){
  ...
  const R=list.map(([e,g])=>{
    ...
    const unit=(OV.dreUnit&&OV.dreUnit[e])||'sc';
  ...
  const revisao=(()=>{ const it=[];
    ...
    R.forEach(r=>{ if(r.unit==='sc' && r.prodHa>1000) it.push(`<b>${esc(r.e)}</b>: produtividade ${nf0.format(r.prodHa)}/ha com unidade “sc” — se não for saca, troque a unidade (ex.: pontos, kg) ao lado da produtividade.`); });
  ...
  const body=R.map(r=>`<tr><td class="c-full"><b>${esc(r.e)}</b></td>
    ...
    <td class="num" data-th="Produt./ha"><span class="dre-prod"><input class="cell ${(OV.dreProdHa&&r.e in OV.dreProdHa)?'edited':''}" inputmode="decimal" data-edit="dreProdHa" data-emp="${esc(r.e)}" value="${r.prodHa}" title="Produção total: ${nf0.format(r.prod)} ${esc(r.unit)}"><input class="cell dre-unit ${r.unit!=='sc'?'edited':''}" data-edit="dreUnit" data-emp="${esc(r.e)}" value="${esc(r.unit)}" title="unidade (sc, pontos, kg…)"></span></td>
...
function applyEdit(el){
  ...
  else if(kind==='dreUnit'){ const e=el.dataset.emp; OV.dreUnit=OV.dreUnit||{}; const u=el.value.trim(); if(!u||u.toLowerCase()==='sc') delete OV.dreUnit[e]; else OV.dreUnit[e]=u; }
