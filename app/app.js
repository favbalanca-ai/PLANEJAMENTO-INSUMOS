...
const APP_VERSION = '2026.07.06-24';   // mostrado no rodapé; ajude a confirmar se a atualização chegou
...
const cotaEmpSel = new Set();   // empreendimentos selecionados no filtro da Cotação (vazio = todos) — sessão
V.compras = function(){
...
document.addEventListener('click',e=>{
  ...
  const ef=e.target.closest('#cot-empf .chip-f');
  if(ef){ const v=ef.dataset.empf;
    if(v===''){ cotaEmpSel.clear(); } else if(cotaEmpSel.has(v)){ cotaEmpSel.delete(v); } else { cotaEmpSel.add(v); }
    route(); return; }
