...
const APP_VERSION = '2026.07.28-70';   // mostrado no rodapé; ajude a confirmar se a atualização chegou
...
function recomCreate(talId, opKey){
  ...
  const r={ id:recomNovoId(), talhao:talId, opKey:opKey||'', opNome:(o&&o.op&&o.op.nome)||'',
    data:new Date().toISOString().slice(0,10), janela:'', alvo:'', calda:'', adjuvante:'', cond:'', resp:'', obs:'',
...
V.recomendacao=function(arg){
  ...
  const opBtns=ops.map(o=>`<button class="btn btn-outline btn-sm" data-act="recomFromOp" data-t="${esc(selId)}" data-op="${esc(o.key)}">➕ ${esc(o.op.nome)}${o.seq==='safrinha'?' · 2ª':''}</button>`).join('')
