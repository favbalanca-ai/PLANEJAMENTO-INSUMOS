...
const APP_VERSION = '2026.07.28-110';   // mostrado no rodapé; ajude a confirmar se a atualização chegou
...
// data planejada da operação = plantio (ou plantio_safrinha) + DAE
function opDataPlan(tid,tagoi,opDap){
  const seq=tagoi[0]==='S'?'safrinha':'principal', pl=planoDe(tid)||{};
  const plantio=(seq==='safrinha')?(pl.plantio_safrinha||pl.plantio||''):(pl.plantio||'');
  const dae=opDaeDe(tid,tagoi,opDap);
  if(!/^\d{4}-\d{2}-\d{2}/.test(plantio)) return '';
  const d=new Date(plantio.slice(0,10)+'T12:00:00'); if(dae) d.setDate(d.getDate()+dae);
...
V.equipe=function(){
  ...
  <div class="panel"><div class="panel-head"><h2>Equipe (RH / SST)</h2><span class="sub">${sst.length} ativo(s) · puxada do sistema de RH</span></div>
    <div class="table-wrap"><table><thead><tr><th>Nome</th><th>Função</th><th>Setor</th><th class="num">Tarefas ativas</th><th class="num">Total</th><th></th></tr></thead>
