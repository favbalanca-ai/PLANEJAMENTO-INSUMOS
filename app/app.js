...
const APP_VERSION = '2026.07.28-65';   // mostrado no rodapé; ajude a confirmar se a atualização chegou
...
const CAMPO_TIPOS={ operacao:{lbl:'Operação',ico:'✅',cor:'#2e7d32'}, monitoramento:{lbl:'Monitoramento',ico:'🐛',cor:'#b7791f'},
...
function camposEventos(){
  ...
  (RECOM&&RECOM.registros||[]).forEach(r=>ev.push({ts:_tsC(r),data:r.data,tipo:'recomendacao',talhao:r.talhao,
    titulo:'Recomendação'+(r.janela?' · '+r.janela:''), resumo:[r.alvo, r.produtos].filter(Boolean).join(' — ')}));
...
V.relatorios = function(){
  ...
  const byTal={}; tals.forEach(t=>byTal[t.id]={monit:0,stand:0,recom:0,oper:0,ult:0});
  ev.forEach(e=>{ const b=byTal[e.talhao]; if(!b) return;
    if(e.tipo==='monitoramento')b.monit++; else if(e.tipo==='stand')b.stand++; else if(e.tipo==='recomendacao')b.recom++; else if(e.tipo==='operacao')b.oper++;
  ...
  const rows=tals.map(t=>{ const b=byTal[t.id];
    return `<tr><td class="c-full"><b>${esc(t.id)}</b>${t.nome?` <span class="mut">${esc(t.nome)}</span>`:''}</td>
      ...
      <td class="num" data-th="Recom.">${b.recom||'·'}</td><td class="num" data-th="Operações">${b.oper||'·'}</td>
  ...
  <div class="panel"><div class="panel-head"><h2>Resumo por talhão</h2><span class="sub">atividade de campo na safra</span></div>
    <div class="table-wrap"><table class="cards-sm"><thead><tr><th>Talhão</th><th class="num">Monit.</th><th class="num">Stand</th><th class="num">Recom.</th><th class="num">Operações</th><th class="num">Última atividade</th></tr></thead>
    <tbody>${rows||'<tr><td colspan="6" class="mut" style="padding:14px">Sem talhões.</td></tr>'}</tbody></table></div></div>`;
};
