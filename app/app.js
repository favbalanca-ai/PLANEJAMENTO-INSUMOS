...
const APP_VERSION = '2026.07.28-56';   // mostrado no rodapé; ajude a confirmar se a atualização chegou
...
function prListaModal(p){
  ...
  ov.innerHTML=`<div class="modal-box">
    ...
    <div class="modal-foot" style="flex-wrap:wrap;gap:8px">
      ...
      <button class="btn btn-primary btn-sm" data-act="prListaDo">Importar ${p.items.length}</button>
    </div></div>`;
  document.body.appendChild(ov);
}
...
function prListaApply(){
  ...
  let msg;
  if(mode==='replace'){ s.itens=p.items.slice(); msg=p.items.length+' produtos (substituído)'; }
  else if(mode==='add'){ s.itens=(s.itens||[]).concat(p.items); msg=p.items.length+' produtos adicionados'; }
  else { const r=_mergePortfolio(s, p.items, true); msg=r.upd+' atualizados · '+r.add+' novos'; }
  savePrecos();
