...
const APP_VERSION = '2026.07.28-114';   // mostrado no rodapé; ajude a confirmar se a atualização chegou
...
V.talhao = function(id){
  ...
  <div class="panel"><div class="panel-head"><h2>Culturas do talhão</h2><span class="sub">planeje a 1ª e a 2ª safra (safrinha) no mesmo talhão</span></div>
    ...
    ${(()=>{ const dt=(seq,edPrev,edReal)=>{ const prev=plantioPrevDe(t.id,seq), man=(OV.result[`${t.id}|${seq}`]||{}).plantioReal, auto=plantioRealAutoDe(t.id,seq), real=_isISO(man)?String(man).slice(0,10):'';
      ...
      return `<div class="bulk-add" style="margin-top:8px">
        <span class="mut" style="font-size:12px;font-weight:700">1ª</span>${dt('principal','plantio','plantioReal')}
        ${temSafrinha(t)?`<span style="width:1px;align-self:stretch;background:var(--line);margin:0 4px"></span><span class="mut" style="font-size:12px;font-weight:700">2ª</span>${dt('safrinha','plantioSaf','plantioRealSaf')}`:''}
...
V.campo = function(arg){
  ...
  const opsHtml=ops.map(o=>{
    ...
    return `<div class="camp-op ${st.cls}">
      ...
      ${(()=>{ const dp=opDataPlan(t.id,o.tagoi,o.op.dap); const dae=opDaeDe(t.id,o.tagoi,o.op.dap); return dp?`<div class="camp-plan">📅 Planejado: <b>${fmtDataBR(dp)}</b>${dae?` <span class="mut">(${dae} DAE)</span>`:''}</div>`:(dae?`<div class="camp-plan mut">Planejado: ${dae} DAE (defina o plantio no talhão)</div>`:''); })()}
...
function reconcileOverrides(){
  ...
  Object.keys(OV.talhao).forEach(id=>{ const o=OV.talhao[id], t=DATA.talhoes.find(x=>x.id===id); if(!t) return;
    ...
    const pl=planoDe(id)||{};   // datas de plantio vivem na aba do talhão
