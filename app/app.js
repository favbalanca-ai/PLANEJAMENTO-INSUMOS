...
const APP_VERSION = '2026.07.06-23';   // mostrado no rodapé; ajude a confirmar se a atualização chegou
...
// empSet (opcional): Set de empreendimentos — soma só as culturas selecionadas (fracionar compras)
function calcDemanda(empSet){
  const dem = {};
...
function calcCompras(empSet){
  const dem = calcDemanda(empSet);
  const rows = [];
...
const comprasEmpSel = new Set();   // empreendimentos selecionados no filtro (vazio = todos) — sessão
V.compras = function(){
  const sel=comprasEmpSel;
  const all=calcCompras(sel.size?sel:null);
  ...
  <div class="toolbar"><div class="search"><input id="q-compra" placeholder="Buscar produto, classe ou fornecedor…"></div>
    <div class="spacer"></div><span class="badge badge-muted">${sel.size?'Demanda só das culturas selecionadas (estoque/pedido são globais). ':''}A comprar = máx(0; Demanda − Estoque − Em pedido).</span></div>
...
document.addEventListener('click',e=>{
  ...
  const ef=e.target.closest('#compras-empf .chip-f');
  if(ef){ const v=ef.dataset.empf;
    if(v===''){ comprasEmpSel.clear(); } else if(comprasEmpSel.has(v)){ comprasEmpSel.delete(v); } else { comprasEmpSel.add(v); }
    route(); return; }
