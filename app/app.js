...
const APP_VERSION = '2026.07.28-55';   // mostrado no rodapé; ajude a confirmar se a atualização chegou
...
function prListaModal(p){
  ...
  ov.innerHTML=`<div class="modal-box">
    ...
    <div class="modal-foot">
      <label class="mut" style="font-size:12px;display:flex;align-items:center;gap:5px"><input type="checkbox" id="pr-lista-replace" checked> Substituir o portfólio atual</label>
      <span class="spacer"></span>
...
function prListaApply(){
  ...
  const rep=document.getElementById('pr-lista-replace'); const replace=!rep||rep.checked;
  const s=safraAtual();
  ...
  s.itens = replace ? p.items : (s.itens||[]).concat(p.items);
  savePrecos();
  const ov=document.getElementById('pr-lista-ov'); if(ov) ov.remove();
  route(); toast('Importado: '+p.items.length+' produtos · '+p.refs.length+' referências');
...
V.precos = function(){
  ...
  idxed.forEach(({it,i})=>{
    ...
    const semPreco = !(precoFinal(it,'vista')>0);
    itemsList+=`<div class="pr-item" data-search="${esc((it.produto+' '+it.empresa+' '+it.classe).toLowerCase())}" data-noprice="${semPreco?1:0}">
...
function route(opts){
  ...
  if(view==='precos' && (precoQ || precoSemPreco)) filterPrecos();
