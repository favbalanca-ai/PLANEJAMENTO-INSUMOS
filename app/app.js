...
const APP_VERSION = '2026.07.28-132';   // mostrado no rodapé; ajude a confirmar se a atualização chegou
...
// a "operação de plantio" da safra = a 1ª operação que leva SEMENTE (classe) — é o dia 0 do DAE
function plantingOpOf(tid,seq){
  const tag=seq==='safrinha'?'S':'P';
  const ops=opsOf(tid,seq)||[];
  for(let oi=0;oi<ops.length;oi++){ const tagoi=`${tag}${oi}`;
    if(effItems(tid,tagoi,ops[oi].itens).some(it=>/semente/i.test(it.classe||''))) return tagoi; }
  return '';
}
// realizada AUTOMÁTICA: data em que a operação de plantio foi concluída no Campo
function plantioRealAutoDe(tid,seq){
  const tagoi=plantingOpOf(tid,seq); if(!tagoi) return '';
  const r=OV.realizado&&OV.realizado[`${tid}|${tagoi}`];
  return (r && r.status==='concluido' && _isISO(r.data)) ? String(r.data).slice(0,10) : '';
}
...
V.talhao = function(id){
  ...
  <div class="panel"><div class="panel-head"><h2>Culturas do talhão</h2><span class="sub">planeje a 1ª e a 2ª safra (safrinha) no mesmo talhão</span></div>
    ...
    ${(()=>{ const dt=(seq,edPrev,edReal,edCiclo)=>{ const prev=plantioPrevDe(t.id,seq), man=(OV.result[`${t.id}|${seq}`]||{}).plantioReal, auto=plantioRealAutoDe(t.id,seq), real=_isISO(man)?String(man).slice(0,10):'';
        ...
        <label class="mut" style="font-size:12px">Plantio realizado <input type="date" class="cell${real?' edited':''}" data-edit="${edReal}" data-id="${t.id}" value="${esc(real||auto)}" title="${real?'Ajustado manualmente':(auto?'Automático: data em que a operação de plantio (semente) foi concluída no Campo':'Preenche sozinho ao concluir a operação de plantio no Campo — ou informe aqui')}" style="width:140px">${(!real&&auto)?'<span class="pill pill-buy" title="veio da conclusão no Campo">auto</span>':''}</label>
...
function maybePlantioConcluido(key){
  ...
  if(plantingOpOf(tid,seq)!==tagoi) return;
...
V.timeline = function(arg){
  ...
  const secs=seqs.map(seq=>{
    ...
    const aviso=!pl?`<p class="lt-aviso">Informe a <b>data de plantio</b> no planejamento do talhão para posicionar operações e estádios na linha do tempo.</p>`:
      ...
      !plReal?`<p class="lt-aviso">🌱 Os <b>estádios</b> começam a ser informados a partir do <b>plantio realizado</b> — ao concluir a operação de plantio no Campo (ou informar a data realizada no planejamento).</p>`:
