...
function precoCompostoIn(it, tipo, refs){
  const ref=(refs||[]).find(r=>(r.classe||'').toUpperCase().trim()===(it.classe||'').toUpperCase().trim());
  if(!ref) return 0;
...
function precoComposto(it, tipo){ return precoCompostoIn(it, tipo, safraAtual().refs); }
// preço final: usa o preço DIRETO do produto (se informado); senão o composto (referência × (1+%))
function precoFinalIn(it, tipo, refs){
  const d = tipo==='prazo' ? it.precoPrazo : it.precoVista;
  if(d!=null && String(d).trim()!=='' && +d>0) return +d;
  return precoCompostoIn(it, tipo, refs);
}
function precoFinal(it, tipo){ return precoFinalIn(it, tipo, safraAtual().refs); }
...
async function prHandleExcel(file){
  ...
  try{
    ...
    // layout "LISTA DE PREÇOS" (referência por classe + %)? importa direto, calculando
    const lista=_prParseLista(rows);
    if(lista && lista.refs.length){ _prListaPrev=lista; prListaModal(lista); return; }
...
function prImportModal(initialText){
  ...
  ov.innerHTML=`<div class="modal-box">
    ...
    <div class="modal-foot" style="flex-wrap:wrap;gap:8px">
      <label class="mut" style="font-size:12px;display:flex;align-items:center;gap:6px">Ao importar:
        <select class="sel" id="pr-imp-mode">
          <option value="update">Atualizar (casar por produto)</option>
          <option value="add">Adicionar ao final</option>
          <option value="replace">Substituir tudo</option>
        </select></label>
      <label class="mut" style="font-size:12px;display:flex;align-items:center;gap:5px"><input type="checkbox" id="pr-imp-sopreco"> Só produtos com preço</label>
...
function prDoImport(){
  ...
  const modeEl=document.getElementById('pr-imp-mode'); const mode=modeEl?modeEl.value:'update';
  let items=[];
  ta.value.split('\n').forEach(l=>{
    if(!l.trim()) return;
  ...
  const s=safraAtual();
  const soEl=document.getElementById('pr-imp-sopreco'); let pulados=0;
  if(soEl&&soEl.checked){ const antes=items.length; items=items.filter(it=>precoFinalIn(it,'vista',s.refs)>0); pulados=antes-items.length; }
  let msg;
  if(mode==='replace'){ s.itens=items; msg='Portfólio substituído — '+items.length+' produtos'; }
  else if(mode==='add'){ s.itens=s.itens.concat(items); msg='Adicionados '+items.length+' produtos'; }
  else { const r=_mergePortfolio(s, items, false); msg='Atualizados '+r.upd+' · novos '+r.add; }
  if(pulados) msg+=' · '+pulados+' sem preço ignorados';
  savePrecos();
  const ov=document.getElementById('pr-import-ov'); if(ov) ov.remove();
  route(); toast(msg);
}
/* ---- Importador da "LISTA DE PREÇOS" (layout favbalança) ----
   Tabela principal: EMPRESA · CLASSE · PRODUTO · … · VALOR R$ REFERÊNCIA · %xPRODUTO REF.
   Tabela de referência ao lado: CLASSE · PRODUTO REFERÊNCIA · PREÇO.
   Produtos de classe COM referência entram pelo % (preço = referência × (1+%));
   os SEM referência entram com o preço direto (coluna VALOR). Retorna null se não for esse layout. */
let _prListaPrev=null;
function _prParseLista(rows){
  if(!rows||!rows.length) return null;
  const norm=s=>String(s==null?'':s).toUpperCase().replace(/\s+/g,' ').trim();
  let hIdx=-1;
  for(let i=0;i<Math.min(rows.length,20);i++){ const r=(rows[i]||[]).map(norm);
    if(r.some(c=>c==='EMPRESA')&&r.some(c=>c==='CLASSE')&&r.some(c=>c==='PRODUTO')){ hIdx=i; break; } }
  if(hIdx<0) return null;
  const H=(rows[hIdx]||[]).map(norm);
  const firstIdx=pred=>H.findIndex(pred);
  const clsCols=[]; H.forEach((h,j)=>{ if(h==='CLASSE') clsCols.push(j); });
  const cEmp=firstIdx(h=>h==='EMPRESA'||h.indexOf('FORNECED')>=0||h.indexOf('FABRICA')>=0);
  const cClsMain=clsCols.length?clsCols[0]:-1;
  const cProdMain=firstIdx(h=>h==='PRODUTO');
  const cValor=firstIdx(h=>h.indexOf('VALOR')>=0);
  const cPct=firstIdx(h=>h.indexOf('%')>=0);
  const cClsRef=clsCols.length>1?clsCols[1]:-1;
  const cProdRef=firstIdx(h=>h.indexOf('PRODUTO REFEREN')>=0);
  let cPrecoRef=firstIdx(h=>h==='PREÇO'||h==='PRECO');
  if(cPrecoRef<0 && cProdRef>=0) cPrecoRef=cProdRef+1;
  if(cClsMain<0||cProdMain<0) return null;
  if(cClsRef<0||cPrecoRef<0) return null;                 // sem tabela de referência: usa o import genérico
  // 1) referências por classe (só à vista)
  const refs=[], refByCls={};
  for(let i=hIdx+1;i<rows.length;i++){ const r=rows[i]||[];
    const cl=norm(r[cClsRef]); if(!cl || cl in refByCls) continue;
    const rp=_priceLoose(r[cPrecoRef]); if(rp==null||rp<=0) continue;
    const rprod=String((cProdRef>=0?r[cProdRef]:'')||'').trim();
    refByCls[cl]=rp; refs.push({classe:cl, produto:rprod, vista:rp, prazo:0});
  }
  // 2) itens da tabela principal
  const items=[]; let comPct=0, comDireto=0, semPreco=0;
  for(let i=hIdx+1;i<rows.length;i++){ const r=rows[i]||[];
    const prod=String(r[cProdMain]||'').trim(); if(!prod) continue;
    const cl=norm(r[cClsMain]); const emp=String((cEmp>=0?r[cEmp]:'')||'').trim();
    const it={empresa:emp, classe:cl, produto:prod};
    const hasRef=cl in refByCls;
    const pct=cPct>=0?_numLoose(r[cPct]):null;             // fração direta (0,029 = +2,9%)
    const valor=cValor>=0?_priceLoose(r[cValor]):null;
    if(hasRef){ it.pct=(pct!=null?pct:0); comPct++; }
    else if(valor!=null && valor>0){ it.precoVista=Math.round(valor*100)/100; comDireto++; }
    else if(pct!=null){ it.pct=pct; comPct++; }            // % sem referência ainda (ex.: RC) — guarda p/ depois
    else semPreco++;
    items.push(it);
  }
  if(!items.length) return null;
  return {refs, items, comPct, comDireto, semPreco};
}
function prListaModal(p){
  const old=document.getElementById('pr-lista-ov'); if(old) old.remove();
  const ov=document.createElement('div'); ov.id='pr-lista-ov'; ov.className='modal-ov';
  ov.innerHTML=`<div class="modal-box">
    <div class="modal-head"><h3>Importar LISTA DE PREÇOS</h3><button class="icon-btn" data-act="prListaClose" title="Fechar">✕</button></div>
    <div style="font-size:13px">
      <p style="margin:0 0 8px">Reconheci o layout com <b>referência por classe</b> + <b>%</b>:</p>
      <ul style="margin:0 0 8px 18px;padding:0">
        <li><b>${p.refs.length}</b> classe(s) de referência (preço-base)</li>
        <li><b>${p.comPct}</b> produto(s) por <b>%</b> — preço = referência × (1 + %)</li>
        <li><b>${p.comDireto}</b> produto(s) com <b>preço direto</b> (classe sem referência)</li>
        ${p.semPreco?`<li class="mut">${p.semPreco} sem preço (nem % nem valor)</li>`:''}
      </ul>
      <p class="mut" style="font-size:12px;margin:0">Ao trocar o preço de uma referência, todos os produtos daquela classe recalculam sozinhos.</p>
    </div>
    <div class="modal-foot" style="flex-wrap:wrap;gap:8px">
      <label class="mut" style="font-size:12px;display:flex;align-items:center;gap:6px">Ao importar:
        <select class="sel" id="pr-lista-mode">
          <option value="update">Atualizar (casar por produto)</option>
          <option value="replace">Substituir tudo</option>
          <option value="add">Adicionar ao final</option>
        </select></label>
      <label class="mut" style="font-size:12px;display:flex;align-items:center;gap:5px"><input type="checkbox" id="pr-lista-sopreco" checked> Só produtos com preço</label>
      <span class="spacer"></span>
      <button class="btn btn-ghost btn-sm" data-act="prListaClose">Cancelar</button>
      <button class="btn btn-primary btn-sm" data-act="prListaDo">Importar</button>
    </div></div>`;
  document.body.appendChild(ov);
}
// casa por nome de produto e atualiza os campos de preço/%; adiciona os que não existem.
// authoritative=true (lista completa): substitui os campos de preço mesmo p/ vazio;
// authoritative=false (import genérico/PDF): só sobrescreve o que veio preenchido (não zera preço).
function _mergePortfolio(s, incoming, authoritative){
  const key=x=>String(x||'').toUpperCase().replace(/\s+/g,' ').trim();
  const byKey={}; (s.itens=s.itens||[]).forEach((it,idx)=>{ const k=key(it.produto); if(k && !(k in byKey)) byKey[k]=idx; });
  let upd=0, add=0;
  incoming.forEach(nit=>{ const k=key(nit.produto);
    if(k && (k in byKey)){ const cur=s.itens[byKey[k]];
      if(authoritative){ cur.precoVista=(nit.precoVista!=null)?nit.precoVista:null; cur.precoPrazo=(nit.precoPrazo!=null)?nit.precoPrazo:null;
        cur.pct=(nit.pct!=null)?nit.pct:null; cur.pctPrazo=(nit.pctPrazo!=null)?nit.pctPrazo:null; }
      else { if(nit.precoVista!=null) cur.precoVista=nit.precoVista; if(nit.precoPrazo!=null) cur.precoPrazo=nit.precoPrazo;
        if(nit.pct!=null) cur.pct=nit.pct; if(nit.pctPrazo!=null) cur.pctPrazo=nit.pctPrazo; }
      if(nit.empresa) cur.empresa=nit.empresa; if(nit.classe) cur.classe=nit.classe;
      upd++;
    } else { s.itens.push(nit); if(k) byKey[k]=s.itens.length-1; add++; } });
  return {upd, add};
}
function prListaApply(){
  const p=_prListaPrev; if(!p) return;
  const modeEl=document.getElementById('pr-lista-mode'); const mode=modeEl?modeEl.value:'update';
  const s=safraAtual();
  // referências: sempre atualiza/insere por classe
  const byCls={}; (s.refs||[]).forEach(r=>{ byCls[(r.classe||'').toUpperCase().trim()]=r; });
  p.refs.forEach(nr=>{ const k=(nr.classe||'').toUpperCase().trim();
    if(byCls[k]){ byCls[k].vista=nr.vista; if(nr.produto) byCls[k].produto=nr.produto; }
    else { s.refs.push(nr); byCls[k]=nr; } });
  // "só com preço": ignora produtos sem preço direto e sem cálculo por referência
  const soEl=document.getElementById('pr-lista-sopreco'); const soPreco=!soEl||soEl.checked;
  let items=p.items; let pulados=0;
  if(soPreco){ const antes=items.length; items=items.filter(it=>precoFinalIn(it,'vista',s.refs)>0); pulados=antes-items.length; }
  let msg;
  if(mode==='replace'){ s.itens=items.slice(); msg=items.length+' produtos (substituído)'; }
  else if(mode==='add'){ s.itens=(s.itens||[]).concat(items); msg=items.length+' produtos adicionados'; }
  else { const r=_mergePortfolio(s, items, true); msg=r.upd+' atualizados · '+r.add+' novos'; }
  if(pulados) msg+=' · '+pulados+' sem preço ignorados';
  savePrecos();
  const ov=document.getElementById('pr-lista-ov'); if(ov) ov.remove();
  route(); toast('Importação: '+msg+' · '+p.refs.length+' referências');
...
function loadOverrides(){
  ...
  OV.opDae   = OV.opDae   || {};   // "TL|tagOp" -> DAE (dias após emergência/plantio) da operação
  ...
  OV.dreUnit = OV.dreUnit || {};   // empreendimento -> unidade de produção (sc, pontos, kg…)
  OV.dreProdHa = OV.dreProdHa || {}; // empreendimento -> produtividade/ha estimada (na unidade acima)
  ...
  OV.opExtra       = OV.opExtra       || {}; // "TL|tag" -> nº de operações NOVAS além das da planilha (viram blocos lá ao sincronizar)
  OV.opDel         = OV.opDel         || {}; // "TL|tagOp" -> true (operação excluída no app; some da planilha ao sincronizar)
...
function countEdits(){
  return Object.keys(OV.estoque).length + Object.keys(OV.pedido).length + Object.keys(OV.dose).length +
         ...
         Object.keys(OV.opMaq).length + Object.keys(OV.opDae||{}).length + Object.keys(OV.arrend).length + Object.keys(OV.dreCfg||{}).length + Object.keys(OV.dreUnit||{}).length + Object.keys(OV.dreProdHa||{}).length + (OV.diesel!==6.00?1:0) +
         ...
         Object.keys(OV.opDel||{}).length + Object.values(OV.opExtra||{}).reduce((a,v)=>a+(+v||0),0) +
...
// mapa produto -> fornecedor (empresa), a partir da lista de Preços (fonte do fornecedor)
// e do portfólio. Assim o fornecedor aparece na Demanda e na Cotação.
function buildFornecedorMap(){
  const map={};
  const put=(prod,emp)=>{ const nm=String(prod||'').trim(), e=String(emp||'').trim();
    if(nm && e && !map[nm]) map[nm]=e; };
  (DATA.produtos||[]).forEach(p=>put(p.produto,p.empresa));   // portfólio (se tiver)
  try{ const s=safraAtual(); (s&&s.itens||[]).forEach(it=>put(it.produto,it.empresa)); }catch(e){}
  return map;
}
function mergePrecosProdutos(){
  try{
    ...
    const uMap=buildUnitMap(), fMap=buildFornecedorMap();
    s.itens.forEach(it=>{
      const nm=String(it.produto||'').trim(); if(!nm||PROD[nm]) return;
      const p={produto:nm, classe:(it.classe||'').toUpperCase(), un:uMap[nm]||'', empresa:(it.empresa||fMap[nm]||''), preco:+precoFinal(it,'vista')||0, estoque:0, pedido:0, _precos:true};
...
// unidade de venda da cultura: ajuste do usuário > padrão pelo nome (braquiária = % de valor cultural/pontos) > sc
function unidadePadrao(emp){ return /BRAQ|BRACH/i.test(String(emp||''))?'%':'sc'; }
function unidadeCultura(emp){ return (OV.dreUnit&&OV.dreUnit[emp])||unidadePadrao(emp); }
...
// eficiência em % (85). Se veio como fração (0,85 — célula formatada em % na planilha ou digitada assim),
// converte: sem isso o rendimento fica 100× menor e o custo/ha 100× maior.
function eficPct(v){ v=+v||0; return (v>0 && v<=1.5) ? v*100 : v; }
function effMaq(m){
  const larg=maqAttr(m,'largura'), vel=maqAttr(m,'velocidade'), efic=eficPct(maqAttr(m,'eficiencia')), lh=maqAttr(m,'l_h'), rs=rsHmDe(m);
...
// DAE (dias após emergência/plantio) da operação: override do app, senão o DAP da planilha
function opDaeDe(tid,tagoi,opDap){ const k=`${tid}|${tagoi}`; return (OV.opDae && k in OV.opDae) ? (+OV.opDae[k]||0) : (+opDap||0); }
// ---- DATA DE PLANTIO por safra: PREVISTA (planejamento) e REALIZADA (execução) ----
const _isISO=s=>/^\d{4}-\d{2}-\d{2}/.test(String(s||''));
// prevista: override do app (OV.talhao) > planilha (aba do talhão, "Data de plantio"); safrinha cai p/ a 1ª se vazia
function plantioPrevDe(tid,seq){
  const o=OV.talhao[tid]||{}, pl=planoDe(tid)||{};
  if(seq==='safrinha'){ const v=(o.plantio_safrinha!=null)?o.plantio_safrinha:(pl.plantio_safrinha||''); return _isISO(v)?String(v).slice(0,10):(_isISO(pl.plantio)?String(pl.plantio).slice(0,10):''); }
  const v=(o.plantio!=null)?o.plantio:(pl.plantio||''); return _isISO(v)?String(v).slice(0,10):'';
}
// produto de SEMENTE (classe do item ou do cadastro) — é o que faz uma operação/recomendação ser o PLANTIO
function _isSemente(it){ return /semente/i.test((it&&it.classe)||'')||/semente/i.test(((it&&PROD[it.produto])||{}).classe||''); }
function opTemSemente(tid,tagoi){ const seq=tagoi[0]==='S'?'safrinha':'principal', op=(opsOf(tid,seq)||[])[+tagoi.slice(1)];
  return !!op && effItems(tid,tagoi,op.itens).some(it=>it.produto&&_isSemente(it)); }
// a 1ª operação da safra que leva SEMENTE (referência do dia 0 do DAE)
function plantingOpOf(tid,seq){
  const tag=seq==='safrinha'?'S':'P', ops=opsOf(tid,seq)||[];
  for(let oi=0;oi<ops.length;oi++) if(opTemSemente(tid,`${tag}${oi}`)) return `${tag}${oi}`;
  return '';
}
// safra de uma recomendação: pela operação vinculada, senão pela cultura informada
function recomSeq(r){ if(r.opKey){ const tg=r.opKey.split('|')[1]||''; return tg[0]==='S'?'safrinha':'principal'; }
  const t=findTalhao(r.talhao); if(!t||!temSafrinha(t)) return 'principal';
  const nk=x=>String(x||'').trim().toUpperCase(), c=nk(r.cultura);
  return (c && c===nk(empSafDe(t)) && c!==nk(empDe(t)))?'safrinha':'principal'; }
const _dISO=ts=>{ const d=new Date(ts); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`; };
// realizada AUTOMÁTICA: o plantio vira REALIZADO quando a RECOMENDAÇÃO DE SEMENTE é concluída (aprovada),
// ou quando uma operação com semente é concluída no Campo — vale a data mais antiga (replantio não move o dia 0)
function plantioRealAutoDe(tid,seq){
  const tag=seq==='safrinha'?'S':'P', datas=[];
...
// realizada EFETIVA: ajuste manual (sincroniza junto com Resultados) > automática
function plantioRealDe(tid,seq){
  const m=OV.result&&OV.result[`${tid}|${seq}`]&&OV.result[`${tid}|${seq}`].plantioReal;
  return _isISO(m)?String(m).slice(0,10):plantioRealAutoDe(tid,seq);
}
// data-base do DAE: realizada (se o plantio já aconteceu) senão prevista
function plantioEffDe(tid,seq){ return plantioRealDe(tid,seq)||plantioPrevDe(tid,seq); }
// ciclo da cultura (dias): override do app > aba do talhão ("Ciclo (dias):")
function cicloDe(tid,seq){ const o=OV.talhao[tid]||{}, pl=planoDe(tid)||{}, k=seq==='safrinha'?'ciclo_safrinha':'ciclo';
  const v=(o[k]!=null)?o[k]:pl[k]; return +v||0; }
// colheita estimada = plantio (realizado > previsto) + ciclo — se atualiza sozinha quando o plantio é concluído
function colheitaPrevDe(tid,seq){ const p=plantioEffDe(tid,seq), c=cicloDe(tid,seq); if(!p||!c) return '';
  const d=new Date(p+'T12:00:00'); d.setDate(d.getDate()+c); return d.toISOString().slice(0,10); }
// data planejada da operação = plantio (realizado > previsto) + DAE
function opDataPlan(tid,tagoi,opDap){
  const seq=tagoi[0]==='S'?'safrinha':'principal';
  const plantio=plantioEffDe(tid,seq), dae=opDaeDe(tid,tagoi,opDap);
  if(!plantio) return '';
  const d=new Date(plantio+'T12:00:00'); if(dae) d.setDate(d.getDate()+dae);
...
function custoOpHa(tid,tag,oi,op){
  // posição VAZIA (sem insumo) e sem máquina escolhida não é uma passada — antes contava o custo médio
  // por passada em cada slot vazio do modelo de 12 operações e inflava a máquina/ha da DRE
  if(!(opMaqKey(tid,`${tag}${oi}`) in OV.opMaq) && !effItems(tid,`${tag}${oi}`,(op&&op.itens)||[]).length) return 0;
...
// área (ha) em que cada produto é aplicado, no escopo do filtro — para a dose/ha média
// (dose/ha = demanda ÷ área de uso). Cada ocorrência do produto numa operação soma a área do talhão,
// batendo com o jeito que a demanda é somada (Σ dose × área).
function areaUsoProduto(empSet, talSet){
  const areaOcc={};
  for(const t of talhoesAll()){
    if(talSet && talSet.size && !talSet.has(t.id)) continue;
    const area=areaDe(t);
    eachOp(t,(seq,tag,oi,op,tagoi)=>{
      if(empSet && empSet.size){ const cult=(seq==='safrinha'?empSafDe(t):empDe(t))||'—'; if(!empSet.has(cult)) return; }
      effItems(t.id,tagoi,op.itens).forEach(it=>{ if(!it.produto) return; areaOcc[it.produto]=(areaOcc[it.produto]||0)+area; });
    });
  }
  return areaOcc;
}
...
function opsOf(tid, seq){
  ...
  const tag=seq==='safrinha'?'S':'P';
  const base=(planoDe(tid)[seq]||[]).slice();
  const extra=(OV.opExtra&&OV.opExtra[`${tid}|${tag}`])||0;         // operações novas (ainda não existem na planilha)
  for(let i=0;i<extra;i++) base.push({nome:'',itens:[],dap:'',extra:true});
  return base.map((op,oi)=>{
...
function opDisplayOrder(tid, tag, cnt){
  const tags=[]; for(let i=0;i<cnt;i++){ if(OV.opDel&&OV.opDel[`${tid}|${tag}${i}`]) continue; tags.push(`${tag}${i}`); }   // excluídas somem
...
function opPlanDirty(tid,seq){
  const tg=seq==='safrinha'?'S':'P', pref=`${tid}|${tg}`;
  for(const k in (OV.opDel||{})){ if(k.indexOf(pref)===0 && OV.opDel[k]) return true; }   // exclusão pendente = reescreve a safra
  const pl=opPlanOf(tid,seq); if(!pl) return false;
...
function opInsertAfter(tid,tag,tagoi){ const seq=_seqOfTag(tag);
  ...
  const total=opsOf(tid,seq).length, cnt=opsShownCount(tid,tag);
  if(cnt>=total){ OV.opExtra=OV.opExtra||{}; OV.opExtra[`${tid}|${tag}`]=(OV.opExtra[`${tid}|${tag}`]||0)+1; }   // sem slot livre: cria uma operação NOVA (vira bloco na planilha)
...
// excluir uma operação: some do app na hora; ao sincronizar, a safra é reescrita sem ela (planilha = espelho)
function opDelete(tid,tag,tagoi){
  if(hasExecInSafra(tid,tag)){ toast('Não dá para excluir: há operação em execução/concluída nesta safra'); return; }
  const op=(opsOf(tid,_seqOfTag(tag))||[])[+tagoi.slice(1)], n=op?effItems(tid,tagoi,op.itens).length:0;
  if(!ask(`Excluir esta operação${n?` e seus ${n} insumo(s)`:''}? Ela some da planilha ao sincronizar.`)) return;
  OV.opDel=OV.opDel||{}; OV.opDel[`${tid}|${tagoi}`]=true;
  opPlanEnsure(tid,_seqOfTag(tag)); opPlanStamp(tid,_seqOfTag(tag)); route({keepScroll:true}); toast('Operação excluída');
}
...
function reconcileReorder(e){
  ...
  const pref=`${tid}|${tag}`, now=Date.now();
  const oiOf=k=>{ if(k.indexOf(pref)!==0) return null; const n=k.slice(pref.length); return /^\d+$/.test(n)?+n:null; };
  const novo=k=>{ const oi=oiOf(k); if(oi==null) return k; return (oi in oldToNew)?`${pref}${oldToNew[oi]}`:null; };   // null = operação saiu
  // máquina planejada: acompanha a operação; posição que ficou vaga perde a máquina
  if(OV.opMaq){ const nm={}; for(const k in OV.opMaq){ const nk=novo(k); if(nk) nm[nk]=OV.opMaq[k]; } OV.opMaq=nm; }
  // execução do Campo (status, doses, máquina, horários…): acompanha a operação. Posição vaga ganha um registro
  // "limpo" carimbado agora, para o dado antigo que está na planilha não voltar no próximo puxar.
  if(OV.realizado){ const R=OV.realizado, moved={}, old=[];
    Object.keys(R).forEach(k=>{ if(oiOf(k)==null) return; old.push(k); const nk=novo(k); if(nk) moved[nk]=Object.assign({},R[k],{_u:now}); delete R[k]; });
    old.forEach(k=>{ if(!moved[k]) R[k]={status:'pendente',data:'',obs:'',doses:{},extras:[],app:{},_u:now}; });
    Object.assign(R,moved); try{ scheduleRealizadoPush(); }catch(err){} }
  // recomendações e tarefas ligadas à operação
  (typeof RECOM!=='undefined'&&RECOM&&RECOM.registros||[]).forEach(r=>{ if(!r.opKey||oiOf(r.opKey)==null) return; const nk=novo(r.opKey);
    if(nk) r.opKey=nk; else { r.opKey=''; r.opOrfa=true; if(r.status==='aprovada') r.saidaPushed=true; } });
  try{ saveRecom(); }catch(err){}
  let tch=false; (typeof TAREFAS!=='undefined'&&TAREFAS&&TAREFAS.tarefas||[]).forEach(tf=>{ if(!tf.opKey||oiOf(tf.opKey)==null) return; const nk=novo(tf.opKey); tf.opKey=nk||''; tch=true; });
  if(tch) try{ saveTarefas(); }catch(err){}
  [OV.dose,OV.itemAdd,OV.itemRemoved,OV.itemProd,OV.opDae,OV.opDel].forEach(map=>{ if(!map) return;
    for(const k in map){ if(k.indexOf(pref)===0) delete map[k]; } });
  if(OV.opPlan) delete OV.opPlan[`${tid}|${seq}`];
  if(OV.opExtra) delete OV.opExtra[`${tid}|${tag}`];   // as novas já viraram blocos na planilha
  delete revealOps[`${tid}|${tag}`];
}
// registros antigos: máquina gravada na execução de operação AINDA NÃO concluída (pode ter vindo de outra operação
// que ocupava a posição antes de uma reordenação) — sai; vale a do planejamento. Carimba p/ a planilha não devolver.
function limparMaqPendentes(){ const R=(OV&&OV.realizado)||{}, now=Date.now(); let n=0;
  for(const k in R){ const r=R[k]; if(r && r.status!=='concluido' && r.app && r.app.maq!=null){ delete r.app.maq; r._u=now; n++; } }
  if(n){ saveOverrides(); try{ scheduleRealizadoPush(); }catch(e){} }
  return n; }
// máquina mostrada no Campo: a do PLANEJAMENTO (fonte única). Depois de concluída, vale a registrada na execução.
function opMaqCampo(t,o,r){ const plan=opMaqDe(t.id,o.tag,o.oi,o.op)||'';
  return (r && r.status==='concluido' && r.app && r.app.maq) ? r.app.maq : plan; }
...
function opsShownCount(tid, tag){
  const seq=tag==='S'?'safrinha':'principal', all=opsOf(tid,seq);   // inclui as operações novas do app
  let lastFilled=-1;
  all.forEach((op,oi)=>{ if(effItems(tid,`${tag}${oi}`,op.itens).length||op.extra) lastFilled=oi; });
...
V.dashboard = function(){
  ...
  <div class="classe-filter" id="painel-empf" style="margin-bottom:14px">
    <button class="chip-f ${painelEmpSel.size===0?'on':''}" data-empf="">Todos</button>
    ${empsAll.map(e=>`<button class="chip-f ${painelEmpSel.has(e)?'on':''}" data-empf="${esc(e)}">${esc(e)}</button>`).join('')}
  </div>
  <div class="kpi-grid">
    <div class="kpi"><div class="k-label">Área total</div><div class="k-value">${num(areaTotal)} ha</div><div class="k-sub">${tais.length} talhões</div></div>
  ...
  ${(()=>{ const aFazer=compras.filter(r=>r.comprar>0).sort((a,b)=>b.valor-a.valor);
    const totAF=aFazer.reduce((a,r)=>a+r.valor,0); const semP=aFazer.filter(r=>r.preco<=0).length;
    return `<div class="panel"><div class="panel-head"><h2>Compras a fazer</h2><span class="sub">demanda − estoque − em pedido — ${aFazer.length} itens · ${brl0(totAF)}${semP?` · ${semP} sem preço`:''}</span></div>
    <div class="table-wrap"><table><thead><tr><th>Produto</th><th>Classe</th><th class="num">Demanda</th><th class="num">Estoque</th><th class="num">Saldo</th><th class="num">Em pedido</th><th class="num">Comprar</th><th>Un</th><th class="num">Valor</th></tr></thead>
    <tbody>${aFazer.slice(0,30).map(r=>`
      <tr><td><b>${esc(r.produto)}</b></td><td><span class="classe-tag">${esc(r.classe)}</span></td>
      <td class="num">${num(r.demanda)}</td><td class="num">${num(r.estoque||0)}</td><td class="num"><b class="est-saldo ${r.saldo<-0.0001?'neg':(r.saida>0?'ok':'')}">${num(r.saldo||0)}</b></td><td class="num">${num(r.pedido||0)}</td>
      <td class="num"><b>${num(r.comprar)}</b></td><td>${esc(r.un)}</td><td class="num">${r.preco>0?brl(r.valor):'<span style="color:var(--red)">s/ preço</span>'}</td></tr>`).join('')||'<tr><td colspan="9" class="empty" style="padding:16px">Nada a comprar 🎉</td></tr>'}</tbody></table></div>
    ${aFazer.length>30?`<p class="mut" style="font-size:12px;padding:6px 16px">Mostrando os 30 maiores de ${aFazer.length}.</p>`:''}</div>`; })()}
  <div class="panel"><div class="panel-head"><h2>Maiores demandas de compra</h2><span class="sub">por empreendimento</span></div>
    <div class="panel-body">${demEmp.map(([n,v])=>`
      <div class="bar-row"><div class="bl" title="${esc(n)}">${esc(n)}</div>
      <div class="bar-track"><div class="bar-fill" style="width:${v/maxE*100}%"></div></div>
      <div class="bar-val">${brl0(v)}</div></div>`).join('')||'<div class="empty">Sem dados</div>'}</div></div>
...
V.talhao = function(id){
  ...
  function opsHtml(seq,tag,titulo,show){
    ...
    return `<div class="panel"><div class="panel-head"><h2>${titulo}</h2><span class="sub">${order.length} operações · ordene com ▲▼ · ＋ insere · 🗑 exclui</span></div>
    ${order.map((oi,pos)=>{ const op=all[oi];
      ...
      <div class="op-head">
        ...
        <span class="op-reorder">
          ...
          <button class="icon-btn op-mv" data-act="opins" data-id="${t.id}" data-op="${tagoi}" title="Inserir operação abaixo">＋</button>
          <button class="icon-btn op-mv del" data-act="opdel" data-id="${t.id}" data-op="${tagoi}" title="Excluir operação">🗑</button>
        </span>
    ...
    <div class="op-add"><button class="btn btn-primary btn-sm" data-act="addop" data-id="${t.id}" data-tag="${tag}">+ adicionar operação</button>${canReveal?'':' <span class="mut" style="font-size:12px">cria uma operação nova (vira um bloco na planilha ao sincronizar)</span>'}</div>
    </div>`;
  }
  const empOpts=empList().filter(e=>e&&e!=='—').map(e=>`<option value="${esc(e)}">`).join('');
  ...
  <div class="panel"><div class="panel-head"><h2>Culturas do talhão</h2><span class="sub">planeje a 1ª e a 2ª safra (safrinha) no mesmo talhão</span></div>
    ...
    ${(()=>{ const dt=(seq,edPrev,edReal,edCiclo)=>{ const prev=plantioPrevDe(t.id,seq), man=(OV.result[`${t.id}|${seq}`]||{}).plantioReal, auto=plantioRealAutoDe(t.id,seq), real=_isISO(man)?String(man).slice(0,10):'';
        const ciclo=cicloDe(t.id,seq), col=colheitaPrevDe(t.id,seq), pelaReal=!!plantioRealDe(t.id,seq);
        return `<label class="mut" style="font-size:12px">Plantio previsto <input type="date" class="cell" data-edit="${edPrev}" data-id="${t.id}" value="${esc(prev)}" title="Dia 0 do DAE — dirige as datas das operações, tarefas e recomendações" style="width:140px"></label>
        <label class="mut" style="font-size:12px">Plantio realizado <input type="date" class="cell${real?' edited':''}" data-edit="${edReal}" data-id="${t.id}" value="${esc(real||auto)}" title="${real?'Ajustado manualmente':(auto?'Automático: data da recomendação de semente concluída (ou operação de plantio concluída no Campo)':'Preenche sozinho quando a recomendação de semente for concluída — ou informe aqui')}" style="width:140px">${(!real&&auto)?'<span class="pill pill-buy" title="veio da recomendação de semente concluída">auto</span>':''}</label>
        <label class="mut" style="font-size:12px">Ciclo <input class="cell${(OV.talhao[t.id]&&OV.talhao[t.id][seq==='safrinha'?'ciclo_safrinha':'ciclo']!=null)?' edited':''}" inputmode="numeric" data-edit="${edCiclo}" data-id="${t.id}" value="${ciclo||''}" placeholder="dias" title="Ciclo da cultura (dias do plantio à colheita)" style="width:62px"> dias</label>
        <span class="mut" style="font-size:12px">🌾 Colheita estimada: ${col?`<b>${fmtDataBR(col)}</b>${pelaReal?' <span class="pill pill-buy" title="calculada pelo plantio realizado">pelo plantio real</span>':' <span class="mut">(prevista)</span>'}`:'<span class="mut">— informe plantio e ciclo</span>'}</span>`; };
      return `<div class="bulk-add" style="margin-top:8px">
        <span class="mut" style="font-size:12px;font-weight:700">1ª</span>${dt('principal','plantio','plantioReal','ciclo')}
        ${temSafrinha(t)?`<span style="width:1px;align-self:stretch;background:var(--line);margin:0 4px"></span><span class="mut" style="font-size:12px;font-weight:700">2ª</span>${dt('safrinha','plantioSaf','plantioRealSaf','cicloSaf')}`:''}
        <button class="btn btn-outline btn-sm" data-act="reagendar" data-id="${esc(t.id)}" title="Recalcula a data das tarefas (não concluídas) deste talhão = plantio + DAE">↻ Recalcular tarefas por DAE</button>
      </div>
...
let precoSemPreco = false;         // filtro "só sem preço" do Portfólio (Preços) — sessão
let precoClasse = '';              // filtro por classe do Portfólio (Preços) — sessão
// custo de insumos por classe (R$/ha) de um talhão -> { CLASSE: rs_ha }
function custoClasseHa(t){
  const m={};
...
function pushEntrada(c){
  ...
  return syncPost(url, JSON.stringify({__entrada:{id:c.id, fornecedor:c.fornecedor, data:c.data, nf:c.nf, obs:c.obs, itens:c.itens}}))
    .then(()=>{ c.pushed=true; saveCompras(); return true; }).catch(()=>false);
}
function pushEntradasPendentes(){
  const url=syncUrl(); if(!url){ toast('Configure a Sincronização primeiro'); return; }
  const pend=(COMPRAS.registros||[]).filter(c=>!c.pushed);
  if(!pend.length){ toast('Nenhuma compra pendente'); return; }
...
function estoqueSaidas(){
  const m={}; const add=(p,v)=>{ if(p) m[p]=(m[p]||0)+(+v||0); };
  const mov=(DATA&&DATA.movimentacao)||null;
  ...
  if(mov&&mov.saidas){
    ...
    (RECOM&&RECOM.registros||[]).forEach(r0=>{ if(r0.saidaPushed||r0.opKey) return; const r=recomNorm(r0); if(r.status!=='aprovada') return;
      (r.itens||[]).forEach(it=>{ if(it.produto && it.real!=null) add(it.produto, it.real); }); });   // recoms avulsas pendentes
    opBaixas(true);
  } else {                                                                     // sem sincronização: só o local
    (RECOM&&RECOM.registros||[]).forEach(r0=>{ if(r0.opKey) return; const r=recomNorm(r0); if(r.status!=='aprovada') return;
      (r.itens||[]).forEach(it=>{ if(it.produto && it.real!=null) add(it.produto, it.real); }); });
    opBaixas(false);
  }
  return m;
}
...
V.estoque=function(){
  ...
  const classes=Object.keys(clsCount).sort((a,b)=>a.localeCompare(b,'pt'));
...
function _persistEquipe(){ try{ localStorage.setItem(EQUIPE_KEY, JSON.stringify(EQUIPE)); }catch(e){} }
function saveEquipe(){ _persistEquipe(); scheduleTarefasPush(); }
...
function _persistTarefas(){ try{ localStorage.setItem(TAREFAS_KEY, JSON.stringify(TAREFAS)); }catch(e){} }
function saveTarefas(){ _persistTarefas(); scheduleTarefasPush(); }
// ---- sincronização (planilha = fonte compartilhada; puxar substitui, enviar grava tudo) ----
let tarefasPushTimer=null, lastTarefasPushSig='';
function tarefasSig(){ return JSON.stringify({f:(EQUIPE&&EQUIPE.funcionarios)||[], t:(TAREFAS&&TAREFAS.tarefas)||[]}); }
function scheduleTarefasPush(){ if(!syncUrl()||!autoOn()) return; clearTimeout(tarefasPushTimer);
  tarefasPushTimer=setTimeout(()=>{ if(tarefasSig()===lastTarefasPushSig) return; tarefasPush({auto:true}); }, 1500); }
async function tarefasPush(opts){ opts=opts||{}; const url=syncUrl(); if(!url){ if(!opts.auto) toast('Configure a URL de sincronização primeiro'); return; }
  if(syncBusy){ if(opts.auto) scheduleTarefasPush(); else toast('Sincronização ocupada — tente de novo'); return; }
...
function tarefasApplyPulled(pa){
  if(!pa || !EQUIPE || !TAREFAS) return false;
  const f=pa.funcionarios||[], t=pa.tarefas||[];
...
let resultPushTimer=null, lastResultSig='';
function resultSig(){ return JSON.stringify((OV&&OV.result)||{}); }
function scheduleResultPush(){ if(!syncUrl()||!autoOn()) return; clearTimeout(resultPushTimer);
  resultPushTimer=setTimeout(()=>{ if(resultSig()===lastResultSig) return; resultPush({auto:true}); }, 1800); }
async function resultPush(opts){ opts=opts||{}; const url=syncUrl(); if(!url){ if(!opts.auto) toast('Configure a sincronização primeiro'); return; }
  if(syncBusy){ if(opts.auto) scheduleResultPush(); return; }
...
function resultApplyPulled(map){
  if(!map || typeof map!=='object' || !OV) return false;
  ...
  talhoesAll().forEach(t=>{
    ...
    ['principal','safrinha'].forEach(seq=>{ const tag=seq==='safrinha'?'S':'P';
      (opsOf(t.id,seq)||[]).forEach((op,oi)=>{
        ...
        const dap=opDaeDe(t.id,tagoi,op.dap);
        const inicio=opDataPlan(t.id,tagoi,op.dap)||new Date().toISOString().slice(0,10);   // plantio (real > previsto) + DAE
...
// recalcula o início das tarefas (não concluídas) ligadas às operações de um talhão/safra = plantio + DAE
function reagendarTarefasSafra(tid,seq){
  const tag=seq==='safrinha'?'S':'P', ops=planoDe(tid)[seq]||[]; let n=0;
  (TAREFAS.tarefas||[]).forEach(tf=>{ if(!tf.opKey||tf.opKey.indexOf(`${tid}|${tag}`)!==0) return;
    if(tf.status==='concluida'||tarefaStatusEff(tf)==='concluida') return;
    const tagoi=tf.opKey.slice(tid.length+1), oi=+tagoi.slice(1), op=ops[oi]; if(!op) return;
    const dt=opDataPlan(tid,tagoi,op.dap); if(dt && dt!==tf.inicio){ tf.inicio=dt; n++; } });
  if(n) saveTarefas();
  return n;
}
// ao concluir uma operação no Campo: se for a operação de PLANTIO da safra (dia 0), a data real vira
// o plantio realizado (automático) e as demais tarefas da safra recalculam por DAE
function maybePlantioConcluido(key){
  const i=key.indexOf('|'); if(i<0) return; const tid=key.slice(0,i), tagoi=key.slice(i+1), seq=tagoi[0]==='S'?'safrinha':'principal';
  if(!opTemSemente(tid,tagoi)) return;
  const man=OV.result&&OV.result[`${tid}|${seq}`]&&OV.result[`${tid}|${seq}`].plantioReal; if(_isISO(man)) return;   // ajuste manual manda
  const real=plantioRealAutoDe(tid,seq); if(!real) return;
  const n=reagendarTarefasSafra(tid,seq);
  toast(`🌱 Plantio realizado em ${fmtDataBR(real)}${n?` — ${n} tarefa(s) recalculada(s) por DAE`:''}`);
}
...
function maybePlantioRecom(r){
  ...
  const real=plantioRealAutoDe(tid,seq); if(!real) return;
  const n=reagendarTarefasSafra(tid,seq);
  setTimeout(()=>toast(`🌱 Plantio realizado em ${fmtDataBR(real)}${n?` — ${n} tarefa(s) recalculada(s) por DAE`:''}`),1200);
}
...
V.equipe=function(){
  ...
  const linha=(f,rem)=>{ const n=ts.filter(t=>t.funcionarioId===f.id).length, ativas=ts.filter(t=>t.funcionarioId===f.id&&t.status!=='concluida').length;
    return `<tr><td><span class="kb-dot" style="background:${funcCor(f.id)}"></span> <b>${esc(f.nome)}</b></td>
      <td>${esc(f.funcao||'—')}</td><td>${esc(f.setor||'—')}</td><td class="num">${ativas}</td><td class="num">${n}</td>
      <td>${rem?`<button class="icon-btn del" data-act="funcDel" data-id="${esc(f.id)}" title="Remover">🗑</button>`:'<span class="mut" title="Vem do sistema de RH / SST">🔒</span>'}</td></tr>`; };
  const sstRows=sst.map(f=>linha(f,false)).join('');
  const manRows=manual.map(f=>linha(f,true)).join('');
  return `
  <div class="panel"><div class="panel-head"><h2>Equipe (RH / SST)</h2><span class="sub">${sst.length} ativo(s) · puxada do sistema de RH</span></div>
    <div class="table-wrap"><table><thead><tr><th>Nome</th><th>Função</th><th>Setor / Unidade</th><th class="num">Tarefas ativas</th><th class="num">Total</th><th></th></tr></thead>
      <tbody>${sstRows||'<tr><td colspan="6" class="mut" style="padding:14px">Sem integrantes do RH. Sincronize (🔄) — e confira se o <b>ID da planilha de RH (SST)</b> está preenchido no Code.gs (<code>SST_DB_ID</code>).</td></tr>'}</tbody></table></div></div>
  <div class="panel"><div class="panel-head"><h2>Integrantes manuais</h2><span class="sub">adicionados no app (avulsos) · ${manual.length}</span></div>
    <div class="app-grid" style="padding:12px 14px">
...
V.cotacao = function(){
  ...
  const areaOcc=areaUsoProduto(sel.size?sel:null);                   // área de uso p/ dose/ha
  const doseHaDe=r=>{ const a=areaOcc[r.produto]||0; return a>0 ? (r.demanda||0)/a : 0; };
  const groups={};
...
V.precos = function(){
  ...
  const pkey=x=>String(x||'').toUpperCase().replace(/\s+/g,' ').trim();
  // safra anterior (para comparar preços): a que vem antes da atual na ordem
  const prevName=(()=>{ const i=safras.indexOf(d.atual); return i>0?safras[i-1]:null; })();
  const prevSafra=prevName?d.safras[prevName]:null;
  const prevMap={};
  if(prevSafra){ (prevSafra.itens||[]).forEach(pit=>{ if(!pit.produto) return; const p=precoFinalIn(pit,'vista',prevSafra.refs||[]); if(p>0) prevMap[pkey(pit.produto)]=p; }); }
  // classes que têm referência (preço-base à vista > 0) — p/ alertar produtos por % sem referência
  const refClasses={}; (s.refs||[]).forEach(r=>{ if(+r.vista>0) refClasses[normc(r.classe)]=true; });
  ...
  const classesList=Object.keys(clsCount).sort((a,b)=>a.localeCompare(b,'pt'));
  let _lastCls=null, itemsList='', faltaRefCount=0;
  idxed.forEach(({it,i})=>{
    ...
    const finalV=precoFinal(it,'vista');
    const usaPct=(it.pct!=null||it.pctPrazo!=null) && !(+it.precoVista>0);
    const faltaRef=usaPct && !refClasses[c];             // depende de % mas a classe não tem referência
    if(faltaRef) faltaRefCount++;
    const semPreco=!(finalV>0);
    // variação vs safra anterior (preço final à vista)
    let varBadge='';
    const pv=prevMap[pkey(it.produto)];
    if(pv>0 && finalV>0){ const dd=(finalV-pv)/pv;
      if(Math.abs(dd)>=0.005){ const up=dd>0; varBadge=` <span class="pr-var ${up?'up':'down'}" title="${esc(prevName||'')}: ${fmtn(pv)} → ${fmtn(finalV)}">${up?'▲':'▼'}${Math.abs(dd*100).toFixed(0)}%</span>`; }
      else varBadge=` <span class="pr-var flat" title="igual à ${esc(prevName||'')}">=</span>`; }
    const finalCell = faltaRef
      ? `<span class="pr-final falta" title="Usa %, mas a classe '${esc(it.classe||'')}' não tem preço de referência — cadastre a referência da classe">⚠ sem ref.</span>`
      : (finalV>0 ? `<span class="pr-final" title="Preço final à vista usado no planejamento">R$ ${fmtn(finalV)}${varBadge}</span>`
                  : `<span class="pr-final zero" title="Sem preço">—</span>`);
    itemsList+=`<div class="pr-item${faltaRef?' pr-falta':''}" data-search="${esc((it.produto+' '+it.empresa+' '+it.classe).toLowerCase())}" data-cls="${esc(c)}" data-noprice="${semPreco?1:0}">
  ...
  <div class="panel"><div class="panel-head"><h2>Portfólio do ano</h2><span class="sub">por classe · A→Z · preço final = referência × (1 + %)${prevName?` · variação vs ${esc(prevName)}`:''}</span></div>
    <div class="toolbar" style="margin:12px 12px 4px">
      <div class="search"><input id="q-preco" value="${esc(precoQ)}" placeholder="Buscar produto, empresa ou classe…" autocomplete="off"></div>
      <button class="chip-f${precoSemPreco?' on':''}" data-act="prToggleSemPreco" title="Mostrar só produtos sem preço à vista">só sem preço</button>
      ${faltaRefCount?`<span class="badge" style="background:var(--red-soft,#fde8e8);color:var(--red)" title="Produtos que usam % mas a classe não tem referência cadastrada">⚠ ${faltaRefCount} sem referência</span>`:''}
      <div class="spacer"></div>
      <span class="badge badge-muted"><b id="pr-count">${idxed.length}</b> de ${idxed.length} itens</span>
    </div>
    <div class="classe-filter" id="pr-clsf" style="margin:2px 12px 8px">
      <button class="chip-f${precoClasse===''?' on':''}" data-clsf="">Todas</button>
      ${classesList.map(c=>`<button class="chip-f${precoClasse===c?' on':''}" data-clsf="${esc(c)}">${esc(c==='—'?'(sem classe)':c)} <span style="opacity:.55">${clsCount[c]}</span></button>`).join('')}
    </div>
    ...
    <div class="op-add">
      ...
      <button class="btn btn-outline btn-sm" data-act="prBuscarValor" title="Traz o preço da coluna VALOR do planejamento para o Preço à vista">⬇ Buscar preços (VALOR)</button>
...
V.dre = function(){
  ...
  const emps={};
  // agrupa por nome NORMALIZADO (espaços duplos/no fim, travessão, maiúsc./minúsc.) — senão "SOJA  26-27" e
  // "SOJA 26-27" viram duas linhas; guarda as variantes p/ avisar onde corrigir o nome
  const nkEmp=s=>String(s==null?'—':s).replace(/[‐‑‒–—−]/g,'-').replace(/\s+/g,' ').trim().toUpperCase();
  ...
  const R=list.map(([e,g])=>{
    // unidade e produtividade/ha por cultura (estimativa da DRE; sc por padrão)
    const unit=unidadeCultura(e);
    const prodHaBase=g.area>0?g.prod/g.area:0;
    const prodHa=(OV.dreProdHa&&e in OV.dreProdHa)?+OV.dreProdHa[e]:prodHaBase;
    const prod=prodHa*g.area;
    const preco=precoCultura(e), receita=prod*preco;
    const opHaCalc=g.area>0?g.opDefault/g.area:0;   // calculado do planejamento (máquinas das operações)
    const opHa=(e in OV.dreOp)?+OV.dreOp[e]:opHaCalc;
    ...
    const precoEq=prod>0?receitaEq/prod:0;
    const prodEq=(preco>0&&g.area>0)?(receitaEq/preco)/g.area:0;
    const margSeg=receita>0?(receita-receitaEq)/receita*100:0;
    const resHa=g.area>0?resultLiq/g.area:0, custoSaca=prod>0?custoTot/prod:0, margLiq=receita>0?resultLiq/receita*100:0;
    return {e,g,unit,prod,prodHa,preco,opHa,opHaCalc,arrHa,receita,custoMaq,custoVar,custoArr,estrutura,imposto,financeiro,outras,receitaLiq,margContrib,resultOper,resultLiq,custoTot,precoEq,prodEq,margSeg,resHa,custoSaca,margLiq};
  });
  // ---- REVISÃO DOS DADOS: aponta o que distorce a DRE e onde corrigir ----
  const revisao=(()=>{ const it=[];
    R.forEach(r=>{ const vs=Object.keys(r.g.variantes); if(vs.length>1)
      it.push(`<b>Nome de cultura escrito de formas diferentes</b> — unifiquei na linha <b>${esc(r.e)}</b>: ${vs.map(v=>`“${esc(v)}” (${esc(r.g.variantes[v].join(', '))})`).join(' · ')}. Corrija o nome nesses talhões para ficar igual.`); });
    const fr=maquinasAll().filter(m=>{ const v=maqAttr(m,'eficiencia'); return v>0&&v<=1.5; });
    if(fr.length) it.push(`<b>Eficiência de máquina informada como fração</b> (ex.: 0,85 em vez de 85%) — já corrigido no cálculo: ${fr.map(m=>esc(m.conjunto)).join(' · ')}. Ajuste na tela <a class="link" data-go="#/maquinas">Máquinas</a>/planilha.`);
    const caras=maquinasAll().map(m=>({m,e:effMaq(m)})).filter(x=>x.e.custoHa>600);
    if(caras.length) it.push(`<b>Conjunto com custo por passada muito alto</b> (&gt; R$ 600/ha) — confira largura, velocidade, eficiência, L/h e R$/HM: ${caras.map(x=>`${esc(x.m.conjunto)} = ${brl(x.e.custoHa)}/ha (${num(x.e.ha_h)} ha/h)`).join(' · ')}.`);
    R.forEach(r=>{ if(r.e in OV.dreOp && Math.abs(r.opHa-r.opHaCalc)>0.5)
      it.push(`<b>${esc(r.e)}</b>: Máq. R$/ha está <b>fixado à mão</b> em ${brl(r.opHa)} (o planejamento calcula ${brl(r.opHaCalc)}). Apague o campo para voltar ao calculado.`); });
    R.forEach(r=>{ if(r.opHaCalc>1500){ const ops=[];
        r.g.cvs.forEach(cv=>{ (opsOf(cv.t.id,cv.seq)||[]).forEach((op,oi)=>{ const c=custoOpHa(cv.t.id,cv.tag,oi,op); if(c>0) ops.push({t:cv.t.id,n:op.nome,cj:opMaqDe(cv.t.id,cv.tag,oi,op)||'média',c}); }); });
        ops.sort((a,b)=>b.c-a.c);
        it.push(`<b>${esc(r.e)}</b>: máquina calculada ${brl(r.opHaCalc)}/ha — maiores: ${ops.slice(0,3).map(o=>`${esc(o.t)} · ${esc(o.n)} (${esc(o.cj)}) ${brl(o.c)}/ha`).join(' · ')}.`); } });
    R.forEach(r=>{ if(!(r.preco>0)) it.push(`<b>${esc(r.e)}</b> está <b>sem preço de venda</b> — a receita fica zerada.`); });
    R.forEach(r=>{ if(r.unit==='sc' && r.prodHa>1000) it.push(`<b>${esc(r.e)}</b>: produtividade ${nf0.format(r.prodHa)}/ha com unidade “sc” — se não for saca, troque a unidade (ex.: %, kg) ao lado da produtividade.`); });
    if(!impostoPct && !estruturaHa && !financeiroPct) it.push(`<b>Premissas zeradas</b> — impostos sobre venda (ex.: Funrural), estrutura/administrativo e custo financeiro estão em 0: o resultado fica <b>superestimado</b>. Preencha em “Premissas gerenciais”.`);
    if(!it.length) return `<div class="panel"><div class="panel-head"><h2>Revisão dos dados</h2><span class="sub">nenhum problema encontrado</span></div></div>`;
    return `<div class="panel dre-rev"><div class="panel-head"><h2>⚠️ Revisão dos dados</h2><span class="sub">${it.length} ponto(s) que distorcem a DRE</span></div>
      <ul style="margin:0;padding:8px 18px 12px 34px;font-size:12.5px;line-height:1.55">${it.map(x=>`<li style="margin:3px 0">${x}</li>`).join('')}</ul></div>`; })();
  const T=(f)=>R.reduce((s,r)=>s+f(r),0);
  const tA=T(r=>r.g.area), tR=T(r=>r.receita), tOut=T(r=>r.outras), tImp=T(r=>r.imposto);
  ...
  const kpiBody=R.map(r=>`<tr><td class="c-full"><b>${esc(r.e)}</b>${r.unit!=='sc'?` <span class="mut" style="font-weight:500">· ${esc(r.unit)}</span>`:''}</td>
    ...
    <td class="num" data-th="Custo/${esc(r.unit)}">${r.prod>0?brlU(r.custoSaca):'—'}</td>
    <td class="num" data-th="Preço equil.">${r.prod>0?brlU(r.precoEq):'—'}</td>
    <td class="num" data-th="Preço atual">${brlU(r.preco)}</td>
    <td class="num" data-th="Produt. equil.">${r.prodEq>0?num(r.prodEq)+' '+esc(r.unit)+'/ha':'—'}</td>
    <td class="num" data-th="Produt. atual">${num(r.prodHa)+' '+esc(r.unit)+'/ha'}</td>
  ...
  const kpis=`<div class="panel"><div class="panel-head"><h2>Indicadores por cultura</h2><span class="sub">ponto de equilíbrio e margem de segurança</span></div>
    <div class="table-wrap"><table class="cards-sm dre-cards"><thead><tr><th>Cultura</th><th class="num">Resultado/ha</th><th class="num">Margem líq.</th><th class="num">Custo/un.</th><th class="num">Preço equil.</th><th class="num">Preço atual</th><th class="num">Produt. equil.</th><th class="num">Produt. atual</th><th class="num">Margem seg.</th></tr></thead>
    <tbody>${kpiBody||'<tr><td colspan="9" class="mut" style="padding:14px">Sem culturas.</td></tr>'}</tbody></table></div>
    <p class="mut" style="font-size:12px;padding:6px 14px 12px">Preço de equilíbrio = preço mínimo por unidade (saca/ponto…) para o resultado zerar. Produtividade de equilíbrio = quanto precisa colher por ha. Margem de segurança = quanto o preço (ou a produtividade) pode cair antes do prejuízo.</p></div>`;
  ...
  const body=R.map(r=>`<tr><td class="c-full"><b>${esc(r.e)}</b></td>
    ...
    <td class="num" data-th="Produt./ha"><span class="dre-prod"><input class="cell ${(OV.dreProdHa&&r.e in OV.dreProdHa)?'edited':''}" inputmode="decimal" data-edit="dreProdHa" data-emp="${esc(r.e)}" value="${r.prodHa}" title="Produção total: ${nf0.format(r.prod)} ${esc(r.unit)}"><input class="cell dre-unit ${(OV.dreUnit&&r.e in OV.dreUnit)?'edited':''}" data-edit="dreUnit" data-emp="${esc(r.e)}" value="${esc(r.unit)}" title="unidade de venda (sc, %, kg…)"></span></td>
    <td class="num" data-th="Preço (R$/un)"><input class="cell ${(r.e in OV.cultura)?'edited':''}" data-edit="cultura" data-emp="${esc(r.e)}" value="${r.preco}"></td>
    <td class="num" data-th="Receita">${brl0(r.receita)}</td>
    <td class="num" data-th="Custo insumos">${brl0(r.g.ins)}</td>
    <td class="num" data-th="Máq. R$/ha"><input class="cell ${(r.e in OV.dreOp)?'edited':''}" data-edit="dreOp" data-emp="${esc(r.e)}" data-def="${r.opHaCalc.toFixed(2)}" value="${r.opHa.toFixed(2).replace('.',',')}" title="${(r.e in OV.dreOp)?`fixado à mão — calculado: ${brl(r.opHaCalc)}/ha (apague p/ voltar)`:'calculado das máquinas das operações'}"></td>
    <td class="num" data-th="Arrend. R$/ha"><input class="cell ${(r.e in OV.arrend)?'edited':''}" data-edit="arrend" data-emp="${esc(r.e)}" value="${r.arrHa.toFixed(2).replace('.',',')}"></td>
  ...
  const tabela=`<div class="toolbar"><span class="badge badge-muted">Custo total inclui impostos, estrutura e financeiro (das premissas). Campos em azul são editáveis — preço, R$/ha e a <b>produtividade/unidade</b> por cultura (ex.: braquiária em <b>pontos</b>).</span></div>
  <div class="panel"><div class="table-wrap"><table class="cards-sm dre-cards">
    <thead><tr><th>Cultura / Empreendimento</th><th class="num">Área (ha)</th><th class="num">Produt./ha</th><th class="num">Preço (R$/un)</th><th class="num">Receita</th><th class="num">Custo insumos</th><th class="num">Máq. R$/ha</th><th class="num">Arrend. R$/ha</th><th class="num">Custo total</th><th class="num">Resultado líq.</th><th class="num">R$/ha</th><th class="num">Margem</th></tr></thead>
    <tbody>${body}</tbody>
    <tfoot class="tfoot"><tr><td>TOTAL</td><td class="num">${num(tA)}</td><td></td><td></td><td class="num">${brl0(tR)}</td><td class="num">${brl0(tIns)}</td><td></td><td></td><td class="num">${brl0(tCustoTot)}</td><td class="num">${sgn(tResLiq)}</td><td class="num">${brl0(resHaT)}</td><td class="num">${nf1.format(margLiqT)}%</td></tr></tfoot>
  </table></div></div>`;
...
const RECOM_ST={
  rascunho:{lbl:'Rascunho',cls:'rc-rasc',ico:'✏️'},
  enviada:{lbl:'Enviada ao operador',cls:'rc-env',ico:'📲'},
  retorno:{lbl:'Retorno recebido',cls:'rc-ret',ico:'↩️'},
  aprovada:{lbl:'Aplicação aprovada',cls:'rc-apr',ico:'✅'}
};
function recomById(id){ return RECOM.registros.find(x=>x.id===id)||null; }
function recomNovoId(){ return 'r'+Date.now().toString(36)+Math.random().toString(36).slice(2,6); }
// normaliza registros legados (texto livre, sem status/itens) para o novo modelo estruturado
function recomNorm(r){
  if(!r.status) r.status='enviada';
  if(!Array.isArray(r.itens)){
    r.itens=(r.produtos||'').split('\n').map(l=>l.trim()).filter(Boolean).map(l=>{
      const m=l.match(/^(.*?)[\s—\-–:]+([\d.,]+)\s*(\S+)?/);
      if(m){ const p=(m[1]||'').trim(); return {produto:p, un:(PROD[p]&&PROD[p].un)||(m[3]||''), dose:_mmC(m[2]), real:null}; }
      return {produto:l, un:(PROD[l]&&PROD[l].un)||'', dose:0, real:null};
    });
  }
...
function recomSetField(el){
  ...
  const f=el.dataset.recf, i=el.dataset.reci, v=el.value; let rerender=false;
...
function recomCard(r,t){
  ...
  let retBlock='';
  if(retEdit){
    retBlock=`<div class="rc-ret-in"><div class="app-grid" style="padding:8px 12px">
      <label>Quem retornou<input class="txt" data-id="${esc(r.id)}" data-recf="retQuem" value="${esc((r.retorno&&r.retorno.quem)||'')}" placeholder="operador"></label>
      <label>Obs. do retorno<input class="txt" data-id="${esc(r.id)}" data-recf="retObs" value="${esc((r.retorno&&r.retorno.obs)||'')}" placeholder="opcional"></label>
    </div></div>`;
  } else if(r.status==='aprovada' && r.retorno && (r.retorno.quem||r.retorno.obs)){
    retBlock=`<div class="rc-meta"><span>↩️ Retorno: ${esc(r.retorno.quem||'—')}</span>${r.retorno.obs?`<span>${esc(r.retorno.obs)}</span>`:''}</div>`;
  }
  let actions='';
  if(r.status==='rascunho'){
    actions=`<button class="btn btn-wa btn-sm" data-act="recomEnviar" data-id="${esc(r.id)}">📲 Enviar por WhatsApp</button>
      <button class="icon-btn del" data-act="recomDel" data-id="${esc(r.id)}" title="Excluir">🗑</button>`;
  } else if(r.status==='enviada'){
    actions=`<button class="btn btn-primary btn-sm" data-act="recomRetorno" data-id="${esc(r.id)}">↩️ Registrar retorno</button>
      <button class="btn btn-wa btn-sm" data-act="recomWa" data-id="${esc(r.id)}">📲 Reenviar</button>
      <button class="btn btn-ghost btn-sm" data-act="recomReabrir" data-id="${esc(r.id)}">✏️ Editar</button>
      <button class="icon-btn del" data-act="recomDel" data-id="${esc(r.id)}" title="Excluir">🗑</button>`;
  } else if(r.status==='retorno'){
    actions=`<span class="rc-hint">Confira o volume utilizado e aprove:</span>
      <span class="spacer"></span>
      <button class="btn btn-ghost btn-sm" data-act="recomReprovar" data-id="${esc(r.id)}">✖ Reprovar</button>
      <button class="btn btn-primary btn-sm" data-act="recomAprovar" data-id="${esc(r.id)}">✅ Aprovar aplicação</button>`;
  } else if(r.status==='aprovada'){
    actions=`<span class="rc-ok">✅ Registrada no histórico do talhão</span>
      <span class="spacer"></span>
...
V.recomendacao=function(arg){
  ...
  const cards=regs.map(r=>recomCard(r,t)).join('')
...
// ---- Shapefile (.shp + .dbf + .prj), direto ou dentro de .zip ----
// projeção: geográfica (graus) ou Transversa de Mercator/UTM (ex.: SIRGAS 2000 / UTM 23S) lida do .prj
function prjTransform(prj){
  const t=String(prj||'');
  if(!t || !/PROJCS/i.test(t)) return p=>[p[1],p[0]];   // geográfica: [lng,lat] -> [lat,lng]
  if(!/Transverse_Mercator|UTM/i.test(t)) throw new Error('projeção do .prj não suportada (use UTM/SIRGAS ou exporte em KML)');
  const par=n=>{ const m=t.match(new RegExp('PARAMETER\\[\\s*"'+n+'"\\s*,\\s*(-?[\\d.eE+-]+)','i')); return m?+m[1]:null; };
  let lon0=par('central_meridian'), fe=par('false_easting'), fn=par('false_northing'), k0=par('scale_factor');
  const z=t.match(/UTM[ _]?zone[ _]?(\d{1,2})\s*([NS])?/i)||t.match(/UTM[ _](\d{1,2})([NS])/i);
  if(lon0==null && z) lon0=(+z[1])*6-183;
  if(fn==null) fn=(z&&/S/i.test(z[2]||''))||/south/i.test(t)?10000000:0;
  if(fe==null) fe=500000; if(k0==null) k0=0.9996; if(lon0==null) throw new Error('.prj sem meridiano central');
  const a=6378137, f=1/298.257222101, e2=f*(2-f), ep2=e2/(1-e2), e1=(1-Math.sqrt(1-e2))/(1+Math.sqrt(1-e2)), rad=Math.PI/180;
  return ([E,N])=>{ const x=E-fe, y=N-fn, M=y/k0, mu=M/(a*(1-e2/4-3*e2*e2/64-5*e2*e2*e2/256));
    ...
    const s=Math.sin(p1), c=Math.cos(p1), tn=Math.tan(p1), N1=a/Math.sqrt(1-e2*s*s), T1=tn*tn, C1=ep2*c*c, R1=a*(1-e2)/Math.pow(1-e2*s*s,1.5), D=x/(N1*k0);
    const lat=p1-(N1*tn/R1)*(D*D/2-(5+3*T1+10*C1-4*C1*C1-9*ep2)*D**4/24+(61+90*T1+298*C1+45*T1*T1-252*ep2-3*C1*C1)*D**6/720);
    const lon=lon0*rad+(D-(1+2*T1+C1)*D**3/6+(5-2*C1+28*T1-3*C1*C1+8*ep2+24*T1*T1)*D**5/120)/c;
    return [lat/rad, lon/rad]; };
}
function parseDbf(buf,cpg){
  const dv=new DataView(buf), n=dv.getUint32(4,true), hl=dv.getUint16(8,true), rl=dv.getUint16(10,true), fields=[];
  let dec; try{ dec=new TextDecoder(/utf-?8/i.test(cpg||'')?'utf-8':'windows-1252'); }catch(e){ dec=new TextDecoder(); }
  for(let o=32;o<hl-1 && dv.getUint8(o)!==0x0D;o+=32){ const nm=dec.decode(new Uint8Array(buf,o,11)).replace(/\0.*$/,'').trim(); fields.push({nm,len:dv.getUint8(o+16)}); }
  const recs=[];
  for(let i=0;i<n;i++){ const base=hl+i*rl; if(base+rl>buf.byteLength) break; let o=base+1; const r={};
    fields.forEach(fd=>{ r[fd.nm]=dec.decode(new Uint8Array(buf,o,fd.len)).trim(); o+=fd.len; }); recs.push(r); }
  return recs;
}
function parseShp(buf,tr){
  const dv=new DataView(buf), out=[]; let o=100, i=0;
  while(o+8<=buf.byteLength){ const len=dv.getInt32(o+4,false)*2, c=o+8, typ=len>=4?dv.getInt32(c,true):0;
    if(typ===5||typ===15||typ===25){ const np=dv.getInt32(c+36,true), npt=dv.getInt32(c+40,true), parts=[];
      for(let k=0;k<np;k++) parts.push(dv.getInt32(c+44+k*4,true));
      const p0=c+44+np*4, rings=[];
      for(let k=0;k<np;k++){ const a=parts[k], b=k+1<np?parts[k+1]:npt, ring=[]; let sa=0;
        for(let j=a;j<b;j++){ const x=dv.getFloat64(p0+j*16,true), y=dv.getFloat64(p0+j*16+8,true); ring.push([x,y]); }
        for(let j=0;j<ring.length-1;j++) sa+=ring[j][0]*ring[j+1][1]-ring[j+1][0]*ring[j][1];
        rings.push({cw:sa<0, pts:ring}); }
      const outer=rings.some(r=>r.cw)?rings.filter(r=>r.cw):rings;   // anéis horários = contorno externo (buracos saem)
      out.push({idx:i, rings:outer.map(r=>r.pts.map(p=>tr(p)).map(([la,lo])=>[+la.toFixed(7),+lo.toFixed(7)])).filter(r=>r.length>2)}); }
    else out.push({idx:i, rings:[]});
    o=c+len; i++; }
  return out;
}
const _NOME_CAMPO=/^(fieldname|field_name|talhao|talhão|nome|name|field|descricao|descr|gleba|id_talhao)$/i;
function shpToPolys(base,f){ // f: {shp:ArrayBuffer, dbf?, prj?, cpg?}
  let tr=prjTransform(f.prj);
  const shapes=parseShp(f.shp,tr), recs=f.dbf?parseDbf(f.dbf,f.cpg):[];
  if(!f.prj){ const s=shapes.find(x=>x.rings.length), p=s&&s.rings[0][0]; if(p && (Math.abs(p[0])>90||Math.abs(p[1])>180)) throw new Error(`${base}: falta o arquivo .prj (projeção) junto do .shp`); }
  return shapes.filter(s=>s.rings.length).map(s=>{ const r=recs[s.idx]||{}, k=Object.keys(r).find(x=>_NOME_CAMPO.test(x)&&r[x]);
    return {nome:(k?r[k]:'')||(shapes.length>1?`${base} ${s.idx+1}`:base), rings:s.rings, arquivo:base+'.shp'}; });
}
// lê um conjunto de arquivos (vários de uma vez, pasta ou .zip) e junta todos os polígonos
async function limLerArquivos(files){
  const polys=[], shp={}, erros=[];
  const baseOf=n=>String(n).split('/').pop().replace(/\.[^.]+$/,'');
  const addShpPart=(name,ext,data)=>{ const b=baseOf(name).toLowerCase(); (shp[b]=shp[b]||{base:baseOf(name)})[ext]=data; };
  for(const file of files){ const nm=file.name||'', ext=(nm.match(/\.([^.]+)$/)||[])[1]; const e=(ext||'').toLowerCase();
    try{
      if(e==='kmz'||e==='zip'){ const JSZip=await loadJSZip(), zip=await JSZip.loadAsync(file);
        for(const fn of Object.keys(zip.files)){ const zf=zip.files[fn]; if(zf.dir||/__MACOSX/.test(fn)) continue; const ze=((fn.match(/\.([^.]+)$/)||[])[1]||'').toLowerCase();
          if(ze==='kml') polys.push(...parseKmlText(await zf.async('string')).map(p=>Object.assign(p,{arquivo:nm})));
          else if(ze==='geojson'||ze==='json') polys.push(...parseGeoJSON(JSON.parse(await zf.async('string'))).map(p=>Object.assign(p,{arquivo:nm})));
          else if(ze==='shp'||ze==='dbf') addShpPart(fn,ze,await zf.async('arraybuffer'));
          else if(ze==='prj'||ze==='cpg') addShpPart(fn,ze,await zf.async('string')); } }
      else if(e==='shp'||e==='dbf') addShpPart(nm,e,await file.arrayBuffer());
      else if(e==='prj'||e==='cpg') addShpPart(nm,e,await file.text());
      else if(e==='kml'||e==='geojson'||e==='json'||e==='xml'){ const txt=await file.text();
        polys.push(...(/^\s*[{[]/.test(txt)?parseGeoJSON(JSON.parse(txt)):parseKmlText(txt)).map(p=>Object.assign(p,{arquivo:nm}))); }
      // .shx, .sbn, .qmd, .cpg soltos etc.: ignorados (não são necessários)
    }catch(err){ erros.push(`${nm}: ${err.message}`); }
  }
  Object.keys(shp).forEach(k=>{ const f=shp[k]; if(!f.shp){ if(f.dbf) erros.push(`${f.base}: falta o arquivo .shp`); return; }
    try{ polys.push(...shpToPolys(f.base,f)); }catch(err){ erros.push(err.message); } });
  return {polys, erros};
}
async function limImportFiles(fileList){
  const files=[...(fileList||[])]; if(!files.length) return;
  toast(`Lendo ${files.length} arquivo(s)…`);
  try{
    const {polys,erros}=await limLerArquivos(files);
    if(!polys.length){ toast(erros.length?('Não consegui ler: '+erros[0]):'Nenhum polígono encontrado. Use KML, KMZ, GeoJSON, ZIP ou Shapefile (.shp + .dbf + .prj).'); return; }
    const usados=new Set();
    _limImport={arquivo:files.length===1?files[0].name:`${files.length} arquivos`, erros, itens:polys.map(p=>{ let tid=limMatchTalhao(p.nome); if(tid&&usados.has(tid)) tid=''; if(tid) usados.add(tid);
      return {nome:p.nome, arquivo:p.arquivo||'', rings:p.rings, ha:limAreaHa(p.rings), tid}; })};
    route({keepScroll:true}); toast(`${polys.length} polígono(s) lido(s) — confira o talhão de cada um e salve`);
  }catch(e){ toast('Não consegui ler os arquivos: '+e.message); }
}
...
function limSalvarImport(){
  ...
  _limImport.itens.forEach(it=>{ if(!it.tid) return; OV.limites[it.tid]={nome:it.nome, arquivo:it.arquivo||_limImport.arquivo, ha:+it.ha.toFixed(2), coords:limCompact(it.rings), _u:Date.now()}; n++; });
...
// semáforo do talhão pelas operações: 🔴 atrasada · 🟡 vence em até 7 dias / em andamento · 🟢 em dia · ⚪ sem programação
const OPST={late:{cor:'#e53935',lbl:'Operação atrasada',ico:'🔴'},soon:{cor:'#fdd835',lbl:'Próximos 7 dias',ico:'🟡'},ok:{cor:'#43a047',lbl:'Em dia',ico:'🟢'},none:{cor:'#b0bec5',lbl:'Sem programação',ico:'⚪'}};
function talStatusOps(t){
  const hj=new Date(_hojeISO()+'T00:00:00'), late=[], soon=[]; let prox=null, tem=false;
  campoOpsDoTalhao(t).forEach(o=>{ const r=realOf(o.key); if(r&&r.status==='concluido') return;
    const dp=opDataPlan(t.id,o.tagoi,o.op.dap); if(!dp) return; tem=true;
    const dd=Math.round((new Date(dp+'T00:00:00')-hj)/86400000), and=r&&r.status==='andamento', it={nome:o.op.nome+(o.seq==='safrinha'?' (2ª)':''),dd,dp,and};
    if(dd<0 && !and) late.push(it); else if(dd<=7 || and) soon.push(it);
    if(dd>=0 && (!prox||dd<prox.dd)) prox=it; });
  late.sort((a,b)=>a.dd-b.dd); soon.sort((a,b)=>a.dd-b.dd);
  return {nivel:late.length?'late':soon.length?'soon':tem?'ok':'none', late, soon, prox};
}
async function mapaInit(){
  ...
  Object.keys(lims).forEach(tid=>{ const lm=lims[tid], t=findTalhao(tid); if(!lm||!lm.coords) return;
    ...
    pg.bindPopup(`<b>${esc(tid)}${t&&t.nome?' · '+esc(t.nome):''}</b><br>${esc(emp||'sem cultura')} · ${nf2.format(haLim)} ha${st?`<br>🌿 hoje: <b>${esc(st.cod)}</b> · ${esc(st.desc)}`:''}
      ...
      ${so.nivel==='ok'&&so.prox?`<div class="mut" style="font-size:12px">Próxima: ${esc(so.prox.nome)} em ${so.prox.dd} d (${_dtBRc(so.prox.dp).slice(0,5)})</div>`:''}
      <a class="link" data-go="#/campo/${encodeURIComponent(tid)}">Operações</a> · <a class="link" data-go="#/timeline/${encodeURIComponent(tid)}">Timeline</a> · <a class="link" data-go="#/monitoramento/${encodeURIComponent(tid)}">Monitorar</a>`);
...
function camposEventos(){
  const ev=[];
  (MONIT&&MONIT.registros||[]).forEach(r=>ev.push({ts:_tsC(r),data:r.data,tipo:'monitoramento',talhao:r.talhao,
  ...
  talhoesAll().forEach(t=>opsDoTalhao(t).forEach(o=>{ const r=realOf(o.key);
    if(r&&r.status==='concluido') ev.push({ts:_tsC(r),data:r.data||'',tipo:'operacao',talhao:t.id,
      titulo:'Operação concluída', resumo:(o.op&&o.op.nome)?o.op.nome:''}); }));
  ev.sort((a,b)=>(b.ts||0)-(a.ts||0));
  return ev;
}
function tlRow(e){
  const ti=CAMPO_TIPOS[e.tipo]||{lbl:e.tipo,ico:'•',cor:'#8aa'};
  return `<div class="tl-row">
    <div class="tl-ico" style="background:${ti.cor}22;color:${ti.cor}">${ti.ico}</div>
    <div class="tl-body"><div class="tl-top"><b>${esc(e.titulo)}</b><span class="tl-date">${_dtBRc(e.data)||''}</span></div>
      <div class="tl-sub">${e.talhao?`<span class="tl-tal">${esc(_talNomeC(e.talhao))}</span>${e.resumo?' · ':''}`:''}${esc(e.resumo||'')}</div></div></div>`;
}
V.campopainel = function(){
  const tals=talhoesAll();
  ...
  const ult=ev[0];
  const ultTxt=ult?(_diasC(ult.ts)===0?'hoje':(_diasC(ult.ts)===1?'ontem':_diasC(ult.ts)+' dias atrás')):'—';
  return `
  <div class="kpi-grid">
    <div class="kpi accent"><div class="k-label">Operações executadas</div><div class="k-value">${pctDone}%</div><div class="k-sub">${prog.done}/${prog.total}${prog.running?` · ${prog.running} em andamento`:''}</div></div>
    <div class="kpi"><div class="k-label">Monitoramentos · 30 dias</div><div class="k-value">${monit30.length}</div><div class="k-sub">${monit.length} no total</div></div>
    <div class="kpi"><div class="k-label">Chuva · 30 dias</div><div class="k-value">${nf0.format(mm30)} mm</div><div class="k-sub">${chuva[0]?'última '+_dtBRc(chuva[0].data)+' · '+chuva[0].mm+' mm':'sem registro'}</div></div>
    <div class="kpi"><div class="k-label">Talhões com atividade</div><div class="k-value">${talAtiv}</div><div class="k-sub">de ${tals.length} talhões</div></div>
  </div>
  <div class="kpi-grid">
    <div class="kpi"><div class="k-label">Contagens de stand</div><div class="k-value">${stand.length}</div><div class="k-sub">${stand[0]?'última '+_dtBRc(stand[0].data):'—'}</div></div>
    <div class="kpi"><div class="k-label">Recomendações</div><div class="k-value">${recom.length}</div><div class="k-sub">${recom[0]?'última '+_dtBRc(recom[0].data):'—'}</div></div>
    <div class="kpi"><div class="k-label">Última atividade</div><div class="k-value" style="font-size:20px">${ultTxt}</div><div class="k-sub">${ult?CAMPO_TIPOS[ult.tipo].lbl:'nenhuma'}</div></div>
    <div class="kpi"><div class="k-label">Eventos no histórico</div><div class="k-value">${ev.length}</div><div class="k-sub"><a class="link" data-go="#/timeline">ver timeline →</a></div></div>
  </div>
  <div class="panel"><div class="panel-head"><h2>Atividade recente</h2><span class="sub"><a class="link" data-go="#/timeline">ver tudo →</a></span></div>
    <div class="tl">${ev.slice(0,8).map(tlRow).join('')||'<p class="mut" style="padding:16px">Nenhum evento ainda. Registre monitoramentos, chuva, stand e recomendações nos atalhos abaixo.</p>'}</div></div>
  <div class="panel"><div class="panel-head"><h2>Registrar</h2></div>
    <div class="bulk-add" style="flex-wrap:wrap;padding:12px 14px">
      <button class="btn btn-outline btn-sm" data-go="#/campo">🧑‍🌾 Operação de campo</button>
      <button class="btn btn-outline btn-sm" data-go="#/monitoramento">🐛 Monitoramento</button>
      <button class="btn btn-outline btn-sm" data-go="#/chuva">🌧️ Chuva</button>
      <button class="btn btn-outline btn-sm" data-go="#/stand">🌱 Contagem de stand</button>
      <button class="btn btn-outline btn-sm" data-go="#/recomendacao">💊 Recomendação</button>
      <button class="btn btn-outline btn-sm" data-go="#/mapa">🗺️ Mapa</button>
    </div></div>`;
};
...
V.timeline = function(arg){
  ...
  const t=tals.find(x=>x.id===sel), hojeIso=_hojeISO();
  const histAll=(porTal[sel]||[]).filter(e=>e.tipo!=='operacao');      // operações entram pela própria lista (com DAE)
  ...
  const seqs=['principal'].concat(temSafrinha(t)?['safrinha']:[]);
  const plS=plantioEffDe(t.id,'safrinha');
  const seqDoEv=e=>(seqs.length>1 && plS && _isoEv(e)>=plS)?'safrinha':'principal';
  const secs=seqs.map(seq=>{
    const emp=seq==='safrinha'?empSafDe(t):empDe(t), fe=estadiosDe(t,seq), pl=plantioEffDe(t.id,seq), plReal=plantioRealDe(t.id,seq);
    const daeDe=iso=>(pl&&iso)?Math.round((new Date(iso+'T12:00:00')-new Date(pl+'T12:00:00'))/86400000):null;
    const it=[];
    if(pl) it.push({tipo:'marco',data:pl,ord:0,titulo:'🌱 Plantio'+(plReal?' realizado':' previsto'),resumo:esc(emp||''),cls:plReal?'':'fut'});
    fe.list.forEach(s=>it.push({tipo:'estadio',data:s.data,ord:1,cod:s.cod,titulo:s.cod,resumo:esc(s.desc),cls:s.data>hojeIso?'fut':''}));
    ops.filter(o=>o.seq===seq).forEach(o=>{ const r=realOf(o.key), done=r&&r.status==='concluido', and=r&&r.status==='andamento';
      const dp=opDataPlan(t.id,o.tagoi,o.op.dap), dae=opDaeDe(t.id,o.tagoi,o.op.dap), dt=(done&&r.data)?String(r.data).slice(0,10):dp;
      const pr=o.items.map(x=>x.produto), prTxt=pr.slice(0,4).join(', ')+(pr.length>4?` +${pr.length-4}`:'');
      const dd=dt?Math.round((new Date(dt+'T00:00:00')-new Date(hojeIso+'T00:00:00'))/86400000):null;
      const tag=done?'<span class="cp-tag ok">✓ concluída</span>':and?'<span class="cp-tag today">em andamento</span>':
        dd==null?'<span class="cp-tag">sem data</span>':dd<0?`<span class="cp-tag late">atrasada ${-dd} d</span>`:dd===0?'<span class="cp-tag today">hoje</span>':`<span class="cp-tag">em ${dd} d</span>`;
      const ref=`DAE ${dae||0}${done&&dp&&dp!==dt?` · previsto ${_dtBRc(dp)}`:''}`;
      it.push({tipo:done?'operacao':'prevista',data:dt,ord:2,titulo:o.op.nome,tag,ref,resumo:esc(prTxt),cls:done?'':(dd!=null&&dd<0&&!and?'late':'fut'),
        estad:estadioEm(fe,dt)}); });
    histAll.filter(e=>seqDoEv(e)===seq).forEach(e=>{ const ti=CAMPO_TIPOS[e.tipo]||{}; it.push({tipo:e.tipo,data:_isoEv(e),ord:3,titulo:e.titulo,resumo:esc(e.resumo||''),estad:estadioEm(fe,_isoEv(e))}); });
    const col=colheitaPrevDe(t.id,seq);
    if(col) it.push({tipo:'marco',data:col,ord:9,titulo:'🌾 Colheita estimada',resumo:`plantio ${fmtDataBR(pl)} + ${cicloDe(t.id,seq)} dias`,cls:col>hojeIso?'fut':''});
    const semData=it.filter(x=>!x.data), comData=it.filter(x=>x.data).sort((a,b)=>a.data.localeCompare(b.data)||a.ord-b.ord);
    // HTML da seção
    let h='', mes='', hojePos=false;
    const hojeRow=()=>{ const st=estadioEm(fe,hojeIso), d=daeDe(hojeIso);
      return `<div class="lt-today"><span>HOJE · ${_dtBRc(hojeIso)}${d!=null&&d>=0?` · DAE ${d}`:''}${st?` · <b>${esc(st.cod)}</b> estimado`:''}</span></div>`; };
    const row=x=>{ const ti=CAMPO_TIPOS[x.tipo]||TL_EXTRA[x.tipo]||{ico:'•',cor:'#8aa'}, d=daeDe(x.data);
      const srch=esc(((x.titulo||'')+' '+(x.cod||'')+' '+String(x.resumo||'').replace(/<[^>]+>/g,'')).toLowerCase());
      if(x.tipo==='estadio') return `<div class="tl-wrap lt-it lt-st ${x.cls||''}" data-tipo="estadio" data-search="${srch}">
        <div class="lt-date"><b>${_dtBRc(x.data).slice(0,5)}</b><span>DAE ${d}</span></div><div class="lt-dot st"></div>
        <div class="lt-card"><span class="lt-stc">${esc(x.cod)}</span> <span class="lt-std">${x.resumo}</span></div></div>`;
      return `<div class="tl-wrap lt-it ${x.cls||''}" data-tipo="${x.tipo}" data-search="${srch}">
        <div class="lt-date">${x.data?`<b>${_dtBRc(x.data).slice(0,5)}</b><span>${d!=null?`DAE ${d}`:''}</span>`:'<b>—</b>'}</div>
        <div class="lt-dot" style="background:${ti.cor}">${ti.ico}</div>
        <div class="lt-card"><div class="lt-top"><b>${esc(x.titulo)}</b>${x.tag||''}</div>
          ${x.ref||x.estad?`<div class="lt-ref">${x.ref?esc(x.ref):''}${x.estad?`${x.ref?' · ':''}estádio ~${esc(x.estad.cod)}`:''}</div>`:''}
          ${x.resumo?`<div class="lt-sub">${x.resumo}</div>`:''}</div></div>`; };
    comData.forEach(x=>{ if(!hojePos && x.data>hojeIso){ h+=hojeRow(); hojePos=true; }
      const mk=x.data.slice(0,7); if(mk!==mes){ mes=mk; const [y,m]=mk.split('-'); h+=`<div class="tl-month">${_MES_TL[(+m||1)-1]} ${y}</div>`; }
      h+=row(x); });
    if(!hojePos && comData.length) h+=hojeRow();
    if(semData.length) h+=`<div class="tl-month">Sem data definida</div>`+semData.map(row).join('');
    const stHoje=estadioEm(fe,hojeIso);
    const aviso=!pl?`<p class="lt-aviso">Informe a <b>data de plantio</b> no planejamento do talhão para posicionar operações e estádios na linha do tempo.</p>`:
      !fe.F?`<p class="lt-aviso">Estádios fenológicos não disponíveis para “${esc(emp||'—')}”.</p>`:
      !plReal?`<p class="lt-aviso">🌱 Os <b>estádios</b> começam a ser informados a partir do <b>plantio realizado</b> — quando a <b>recomendação de semente</b> for concluída (ou a data realizada for informada no planejamento).</p>`:
      fe.padrao?`<p class="lt-aviso">Ciclo não informado: estádios estimados com o ciclo de referência de ${fe.F.nome} (${fe.F.ref} dias). Informe o ciclo no planejamento para ajustar.</p>`:'';
    return {seq,stHoje,fe,html:`<div class="lt-sec"><b>${seq==='safrinha'?'2ª safra':'1ª safra'} · ${esc(fe.F?fe.F.nome:(emp||'—'))}</b>
        <span>${pl?`plantio ${fmtDataBR(pl)}${plReal?'':' (previsto)'}`:'sem plantio'}${cicloDe(t.id,seq)?` · ciclo ${cicloDe(t.id,seq)} d`:''}${col?` · colheita ~${fmtDataBR(col)}`:''}</span></div>
      ${aviso}<div class="lt">${h||'<p class="mut" style="padding:14px">Nada para mostrar ainda.</p>'}</div>`};
  });
  const todos=[]; secs.forEach(s=>todos.push(...[...s.html.matchAll(/data-tipo="([a-z]+)"/g)].map(m=>({tipo:m[1]}))));
  const stAtual=secs.map(s=>s.stHoje?`<span class="lt-now">🌿 ${esc(s.fe.F.nome)} hoje: <b>${esc(s.stHoje.cod)}</b> · ${esc(s.stHoje.desc)}</span>`:'').join('');
  ...
  return `${top}
    ...
    ${chipsDe(todos,['prevista','estadio','marco'])}
    <div id="tl-list">${secs.map(s=>`<div class="panel lt-panel">${s.html}</div>`).join('')}</div>
    <p class="mut" style="font-size:11px;margin:6px 4px">Estádios estimados a partir da data de plantio e do ciclo da cultura — servem de referência; confirme no campo.</p>`;
};
// tipos que só existem na timeline do talhão
const TL_EXTRA={ prevista:{lbl:'Previstas',ico:'🗓️',cor:'#64757d'}, estadio:{lbl:'Estádios',ico:'🌿',cor:'#558b2f'}, marco:{lbl:'Marcos',ico:'🚩',cor:'#1f4a55'} };
// ===== FENOLOGIA: estádios estimados (dias após o plantio num ciclo de referência; escalam pelo ciclo informado) =====
const FENO={
  soja:{nome:'Soja',ref:120,st:[['VE',6,'emergência'],['VC',10,'cotilédones abertos'],['V1',14,'folhas unifolioladas abertas'],['V2',19,'1º trifólio aberto'],['V3',24,'2º trifólio aberto'],['V4',29,'3º trifólio aberto'],['V5',34,'4º trifólio aberto'],['V6',39,'5º trifólio aberto'],
    ['R1',48,'início do florescimento'],['R2',54,'florescimento pleno'],['R3',62,'início da formação de vagens'],['R4',71,'vagens completamente desenvolvidas'],['R5.1',78,'início do enchimento de grãos (até 10%)'],['R5.2',82,'enchimento 11–25%'],['R5.3',86,'enchimento 26–50%'],['R5.4',90,'enchimento 51–75%'],['R5.5',94,'enchimento 76–100%'],['R6',99,'grão cheio · vagem verde'],['R7',108,'início da maturação'],['R8',116,'maturação plena']]},
  milho:{nome:'Milho',ref:130,st:[['VE',6,'emergência'],['V2',13,'2 folhas desenvolvidas'],['V4',20,'4 folhas'],['V6',28,'6 folhas'],['V8',36,'8 folhas'],['V10',44,'10 folhas'],['V12',51,'12 folhas'],['V14',57,'14 folhas'],['VT',63,'pendoamento'],
    ['R1',67,'embonecamento'],['R2',78,'grão bolha d’água'],['R3',88,'grão leitoso'],['R4',97,'grão pastoso'],['R5',107,'grão farináceo · dente'],['R6',122,'maturidade fisiológica']]},
  sorgo:{nome:'Sorgo',ref:115,st:[['VE',6,'emergência'],['E1',14,'3 folhas'],['E2',22,'5 folhas'],['E3',32,'diferenciação do ponto de crescimento'],['E4',44,'folha bandeira visível'],['E5',54,'emborrachamento'],['E6',63,'florescimento'],['E7',74,'grão leitoso'],['E8',86,'grão pastoso'],['E9',104,'maturidade fisiológica']]},
  feijao:{nome:'Feijão',ref:90,st:[['V1',6,'emergência'],['V2',10,'folhas primárias abertas'],['V3',16,'1ª folha trifoliolada'],['V4',23,'3ª folha trifoliolada'],['R5',32,'pré-floração'],['R6',38,'floração'],['R7',44,'formação de vagens'],['R8',55,'enchimento de vagens'],['R9',78,'maturação']]},
  algodao:{nome:'Algodão',ref:170,st:[['VE',7,'emergência'],['V1',14,'1ª folha verdadeira'],['V4',30,'4 folhas'],['B1',40,'1º botão floral'],['F1',60,'1ª flor'],['F5',75,'flor no 5º ramo'],['C1',115,'1º capulho aberto'],['CM',150,'maturação']]},
  trigo:{nome:'Trigo',ref:110,st:[['EM',7,'emergência'],['PF',25,'perfilhamento'],['AL',40,'alongamento'],['EB',55,'emborrachamento'],['ES',63,'espigamento'],['FL',70,'florescimento'],['GL',80,'grão leitoso'],['GP',90,'grão pastoso'],['MF',104,'maturação']]},
  braquiaria:{nome:'Braquiária',ref:180,st:[['EM',10,'emergência'],['PF',30,'perfilhamento'],['AL',65,'alongamento'],['EP',110,'emissão de panículas'],['FL',125,'florescimento'],['MS',165,'maturação das sementes']]}
};
function culturaKey(emp){ const s=String(emp||'').normalize('NFD').replace(/[̀-ͯ]/g,'').toUpperCase();
  return /SOJA/.test(s)?'soja':/MILHO/.test(s)?'milho':/SORGO/.test(s)?'sorgo':/FEIJ/.test(s)?'feijao':/ALGOD/.test(s)?'algodao':/TRIGO/.test(s)?'trigo':/BRAQ|BRACH/.test(s)?'braquiaria':''; }
function _hojeISO(){ const d=new Date(); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`; }
// estádios de uma safra do talhão: [{cod,desc,dae,data}] — só a partir do PLANTIO REALIZADO: data real + dias × (ciclo/ciclo de referência)
function estadiosDe(t,seq){ const F=FENO[culturaKey(seq==='safrinha'?empSafDe(t):empDe(t))]||null, pl=plantioRealDe(t.id,seq);
  if(!F||!pl) return {F,pl,aguarda:!pl,list:[]};
  const cic=cicloDe(t.id,seq), f=cic?cic/F.ref:1, base=new Date(pl+'T12:00:00');
  return {F,pl,padrao:!cic,list:F.st.map(([cod,d,desc])=>{ const n=Math.round(d*f), x=new Date(base); x.setDate(x.getDate()+n); return {cod,desc,dae:n,data:x.toISOString().slice(0,10)}; })}; }
// estádio estimado numa data (o último já atingido)
function estadioEm(fe,iso){ if(!fe||!fe.list||!fe.list.length||!iso) return null; let c=null; fe.list.forEach(s=>{ if(s.data<=iso) c=s; }); return c; }
function filterTimeline(){
  const list=document.querySelector('#tl-list'); if(!list) return;
  const q=(timelineQ||'').toLowerCase().trim(); let n=0;
  list.querySelectorAll('.tl-wrap').forEach(el=>{
...
V.relatorios = function(){
  const tals=talhoesAll(), ev=camposEventos();
  const byTal={}; tals.forEach(t=>byTal[t.id]={monit:0,stand:0,recom:0,apl:0,oper:0,ult:0});
  ev.forEach(e=>{ const b=byTal[e.talhao]; if(!b) return;
    if(e.tipo==='monitoramento')b.monit++; else if(e.tipo==='stand')b.stand++; else if(e.tipo==='recomendacao')b.recom++;
  ...
  const chuvaTot=(CHUVA&&CHUVA.registros||[]).reduce((a,r)=>a+_mmC(r.mm),0);
  return `
  <div class="toolbar"><span class="badge badge-muted">${tals.length} talhões · ${ev.length} eventos · ${nf0.format(chuvaTot)} mm de chuva registrada</span>
    <div class="spacer"></div><button class="btn btn-outline btn-sm" data-act="relCsv">⬇ Exportar CSV</button></div>
  <div class="panel"><div class="panel-head"><h2>Resumo por talhão</h2><span class="sub">atividade de campo na safra</span></div>
    <div class="table-wrap"><table class="cards-sm"><thead><tr><th>Talhão</th><th class="num">Monit.</th><th class="num">Stand</th><th class="num">Recom.</th><th class="num">Aplic.</th><th class="num">Operações</th><th class="num">Última atividade</th></tr></thead>
    <tbody>${rows||'<tr><td colspan="7" class="mut" style="padding:14px">Sem talhões.</td></tr>'}</tbody></table></div></div>`;
};
function exportCampoCsv(){
  const ev=camposEventos();
  const esc2=s=>`"${String(s==null?'':s).replace(/"/g,'""')}"`;
  const linhas=[['Data','Tipo','Talhão','Título','Detalhe'].map(esc2).join(';')];
  ev.forEach(e=>linhas.push([_dtBRc(e.data), (CAMPO_TIPOS[e.tipo]||{}).lbl||e.tipo, e.talhao?_talNomeC(e.talhao):'', e.titulo, e.resumo].map(esc2).join(';')));
  download('relatorio_campo.csv', '﻿'+linhas.join('\r\n'), 'text/csv');
}
V.mapa=function(){
  ...
  const imp=_limImport?`<div class="panel lim-imp"><div class="panel-head"><h2>📥 Conferir importação</h2><span class="sub">${esc(_limImport.arquivo)}</span></div>
      ...
      ${(_limImport.erros||[]).length?`<div class="lim-errs">⚠️ Não lidos:<br>${_limImport.erros.map(esc).join('<br>')}</div>`:''}
  ...
  const stCount={late:0,soon:0,ok:0,none:0}; comLim.forEach(t=>stCount[talStatusOps(t).nivel]++);
  const legCult=['late','soon','ok','none'].filter(k=>k!=='none'||stCount.none).map(k=>`<span><i style="background:${OPST[k].cor};border-radius:3px"></i>${OPST[k].lbl} <b>${stCount[k]}</b></span>`).join('');
...
function finalizarOpModal(key){
  const i=key.indexOf('|'); if(i<0) return; const talId=key.slice(0,i), t=findTalhao(talId); if(!t){ toast('Talhão não encontrado'); return; }
  const o=opsDoTalhao(t).find(x=>x.key===key); if(!o){ toast('Operação não encontrada'); return; }
  const area=areaDe(t), r=realEnsure(key), agg={};
  effItems(t.id,o.tagoi,o.op.itens).forEach(it=>{ if(!it.produto) return; const iid=it.kind==='base'?String(it.ii):'a'+it.ai;
    const doseHa=(r.doses&&(iid in r.doses)&&r.doses[iid]!=='')?+r.doses[iid]:(+it.dose||0);
    if(!agg[it.produto]) agg[it.produto]={un:it.un||'',vol:0}; agg[it.produto].vol+=doseHa*area; });
  (r.extras||[]).forEach(ex=>{ if(!ex.produto) return; if(!agg[ex.produto]) agg[ex.produto]={un:(PROD[ex.produto]&&PROD[ex.produto].un)||'',vol:0}; agg[ex.produto].vol+=(+ex.dose||0)*area; });
  const rows=Object.keys(agg).map(p=>{ const def=(r.baixa&&r.baixa[p]!=null)?r.baixa[p]:+agg[p].vol.toFixed(2);
    return `<tr><td><b>${esc(p)}</b></td><td class="num"><input class="cell" inputmode="decimal" data-finprod="${esc(p)}" value="${def||''}" placeholder="0"><small> ${esc(agg[p].un)}</small></td></tr>`; }).join('')
    || '<tr><td colspan="2" class="mut" style="padding:8px 10px">Sem insumos nesta operação — pode concluir mesmo assim.</td></tr>';
  ...
  ov.innerHTML=`<div class="modal-box">
    ...
    <div style="overflow:auto;max-height:60vh">
      ...
      <div class="app-grid" style="margin-bottom:8px">
        <label>Operador<input class="txt" data-finmeta="operador" value="${esc((r.app&&r.app.operador)||'')}" placeholder="quem executou"></label>
        <label>Data<input type="date" data-finmeta="data" value="${esc(r.data||new Date().toISOString().slice(0,10))}"></label>
      </div>
      <table class="camp-ins"><thead><tr><th>Insumo</th><th class="num">Volume usado</th></tr></thead><tbody>${rows}</tbody></table>
    </div>
    <div class="modal-foot"><span class="spacer"></span>
      <button class="btn btn-ghost btn-sm" data-act="finClose">Cancelar</button>
      <button class="btn btn-primary btn-sm" data-act="finConfirm" data-key="${esc(key)}">✅ Concluir e dar baixa</button>
    </div></div>`;
  document.body.appendChild(ov);
}
        ...
        <div class="camp-app-in">
          <div class="app-grid">
            ...
            <label>Máquina/aeronave<select class="sel" data-edit="realApp" data-field="maq" data-key="${esc(o.key)}">
              ...
              ${maquinasAll().map(mm=>`<option value="${esc(mm.conjunto)}"${(opMaqCampo(t,o,r)===mm.conjunto)?' selected':''}>${esc(mm.conjunto)}</option>`).join('')}
            </select></label>
...
function applyEdit(el){
  ...
  if(kind==='opDae'){   // DAE (dias) da operação — grava override e vai p/ a coluna DAP da planilha
  ...
  if(kind==='plantio'||kind==='plantioSaf'){   // data de plantio PREVISTA (dia 0 do DAE) — vai p/ a aba do talhão
    const id=el.dataset.id, v=el.value.trim(), pl=planoDe(id)||{};
    const key=kind==='plantio'?'plantio':'plantio_safrinha', base=String(pl[key]||'').slice(0,10);
  ...
  if(kind==='ciclo'||kind==='cicloSaf'){   // ciclo da cultura (dias) -> colheita estimada; vai p/ o resumo da aba do talhão
    const id=el.dataset.id, pl=planoDe(id)||{}, key=kind==='ciclo'?'ciclo':'ciclo_safrinha';
    const v=el.value.trim(), n=v===''?null:(parseInt(v,10)||0), base=+pl[key]||0;
    OV.talhao[id]=OV.talhao[id]||{};
    if(n==null||n===base) delete OV.talhao[id][key]; else OV.talhao[id][key]=n;
    if(!Object.keys(OV.talhao[id]).length) delete OV.talhao[id];
    saveOverrides(); route(); return;
  }
  if(kind==='plantioReal'||kind==='plantioRealSaf'){   // data de plantio REALIZADA (ajuste manual; sincroniza c/ Resultados)
    const id=el.dataset.id, seq=kind==='plantioReal'?'principal':'safrinha', k=`${id}|${seq}`, v=el.value.trim();
    OV.result[k]=OV.result[k]||{};
    if(v===''||v===plantioRealAutoDe(id,seq)) delete OV.result[k].plantioReal; else OV.result[k].plantioReal=v;
    const keys=Object.keys(OV.result[k]).filter(x=>x!=='_u');
    if(!keys.length) delete OV.result[k]; else OV.result[k]._u=Date.now();
    saveOverrides(); scheduleResultPush();
    const n=reagendarTarefasSafra(id,seq); route(); if(n) toast(`Plantio realizado salvo — ${n} tarefa(s) recalculada(s)`); return;
  }
  if(kind==='resProd'||kind==='resPreco'){   // Resultados: colhido/ha e preço de venda (por talhão/safra)
    ...
    const keys=Object.keys(OV.result[k]).filter(x=>x!=='_u');
    if(!keys.length) delete OV.result[k]; else OV.result[k]._u=Date.now();
    saveOverrides(); scheduleResultPush(); route({keepScroll:true}); return;
  }
  ...
  if(kind==='realApp' && el.dataset.field==='maq'){   // máquina: não concluída = muda o PLANEJAMENTO (fonte única)
    const key=el.dataset.key, r0=OV.realizado[key];
  ...
  else if(kind==='dreOp'){ const e=el.dataset.emp, def=parseFloat(el.dataset.def);   // igual ao calculado = não fixa (continua acompanhando o planejamento)
    if(n==null || (isFinite(def) && Math.abs(n-def)<0.005)) delete OV.dreOp[e]; else OV.dreOp[e]=n; }
  ...
  else if(kind==='dreProdHa'){ const e=el.dataset.emp; OV.dreProdHa=OV.dreProdHa||{}; if(n==null) delete OV.dreProdHa[e]; else OV.dreProdHa[e]=n; }
  else if(kind==='dreUnit'){ const e=el.dataset.emp; OV.dreUnit=OV.dreUnit||{}; const u=el.value.trim(); if(!u||u.toLowerCase()===unidadePadrao(e).toLowerCase()) delete OV.dreUnit[e]; else OV.dreUnit[e]=u; }
...
document.addEventListener('click',e=>{
  ...
  const ef=e.target.closest('#cot-empf .chip-f, #compras-empf .chip-f, #painel-empf .chip-f');
  ...
  if(act){
    ...
    if(a.act==='delitem'){ delItem(a); saveOverrides(); route(); }
      ...
      const shown=opsShownCount(a.id,a.tag), seq=a.tag==='S'?'safrinha':'principal', total=opsOf(a.id,seq).length;
      if(shown>=total){ OV.opExtra=OV.opExtra||{}; OV.opExtra[rk]=(OV.opExtra[rk]||0)+1; saveOverrides(); }   // além das da planilha: operação nova
      revealOps[rk]=shown+1; route(); }
    else if(a.act==='opdel'){ opDelete(a.id, a.op[0], a.op); }
      ...
      try{ maybePlantioConcluido(key); }catch(e){}
      ...
      else { const d=editDesc(cur); if(confirm(`Descartar esta edição?\n${d.lbl}: ${d.txt}\n\nVolta a valer o que está na planilha.`)){ if(discardEdit(cur)){ toast('Edição descartada'); updateEditBadge(); route({keepScroll:true}); } } } }
      ...
      if(r.opKey){   // recomendação de uma operação de campo: a baixa é da operação (id op:), sem contar em dobro
        const rl=realEnsure(r.opKey), baixa={};
      ...
      const rec={ id:'c'+Date.now().toString(36)+Math.random().toString(36).slice(2,6), fornecedor:d.fornecedor||'', data:d.data||new Date().toISOString().slice(0,10), nf:d.nf||'', obs:d.obs||'', itens, pushed:false, ts:Date.now() };
    ...
    else if(a.act==='prListaClose'){ const ov=document.getElementById('pr-lista-ov'); if(ov) ov.remove(); }
    else if(a.act==='prListaDo'){ prListaApply(); }
  ...
  if(e.target.id==='btn-dem-emp-pdf') exportDemandaEmpPDF();
});
...
function filterPrecos(){
  const list=document.querySelector('.pr-list'); if(!list) return;
  const q=(precoQ||'').toLowerCase().trim();
  const kids=[...list.children];
  let shown=0;
  kids.forEach(el=>{
    if(!el.classList.contains('pr-item')) return;
    const okQ=!q||(el.dataset.search||'').includes(q);
    const okS=!precoSemPreco||el.dataset.noprice==='1';
    const okC=!precoClasse||(el.dataset.cls||'')===precoClasse;
    const show=okQ&&okS&&okC;
    el.style.display=show?'':'none';
    if(show) shown++;
  });
  // esconde o cabeçalho de uma classe se nenhum item dela estiver visível
  kids.forEach((el,idx)=>{
    if(!el.classList.contains('pr-grp')) return;
    let any=false;
    for(let j=idx+1;j<kids.length;j++){
      if(kids[j].classList.contains('pr-grp')) break;
      if(kids[j].classList.contains('pr-item')&&kids[j].style.display!=='none'){ any=true; break; }
    }
    el.style.display=any?'':'none';
  });
  const cnt=document.querySelector('#pr-count'); if(cnt) cnt.textContent=shown;
}
...
function exportCotacaoCSV(){
  ...
  const areaOcc=areaUsoProduto(cotaEmpSel.size?cotaEmpSel:null);
  let csv='FORNECEDOR;PRODUTO;CLASSE;DOSE_HA;QTD;UN;PRECO_REF;CUSTO_HA;VALOR_REF\n';
  rows.forEach(r=>{ const a=areaOcc[r.produto]||0, dha=a>0?(r.demanda||0)/a:0, cha=dha*(r.preco||0);
    csv+=[r.empresa||'(sem fornecedor)',r.produto,r.classe,nf2.format(dha),num(r.comprar),r.un,
    nf2.format(r.preco),nf2.format(cha),nf2.format(r.valor)].map(x=>`"${String(x).replace(/"/g,'""')}"`).join(';')+'\n';});
...
function exportTalhaoPDF(id){
  ...
  const seqInfo=(seq)=>{ const prev=plantioPrevDe(t.id,seq), real=plantioRealDe(t.id,seq), ciclo=cicloDe(t.id,seq), col=colheitaPrevDe(t.id,seq);
    return [prev?`Plantio previsto <b>${fmtDataBR(prev)}</b>`:'', real?`Plantio realizado <b>${fmtDataBR(real)}</b>`:'', ciclo?`Ciclo <b>${ciclo} dias</b>`:'',
      col?`Colheita estimada <b>${fmtDataBR(col)}</b>${real?' (pelo plantio real)':''}`:''].filter(Boolean).join(' · '); };
  const seqBlock=(seq,tag,cultura,prod)=>{
    const all=opsOf(t.id,seq); if(!all.length) return '';
    const order=opDisplayOrder(t.id,tag,opsShownCount(t.id,tag));   // mesma ordem/nomes que a tela
    const rows=[]; let insHa=0, maqSeq=0;
    order.forEach((oi,pos)=>{ const op=all[oi], tagoi=`${tag}${oi}`, items=effItems(t.id,tagoi,op.itens).filter(it=>it.produto);
      if(!items.length) return;   // não imprime operações vazias
      const conj=opMaqDe(t.id,tag,oi,op), mHa=custoOpHa(t.id,tag,oi,op); let sub=0; items.forEach(it=>sub+=it.dose*precoDe(it.produto));
      rows.push({pos:pos+1, op, items, conj, mHa, sub, dae:opDaeDe(t.id,tagoi,op.dap), dp:opDataPlan(t.id,tagoi,op.dap)}); insHa+=sub; maqSeq+=mHa; });
    if(!rows.length) return '';
    const totSeq=insHa+maqSeq, lbl=seq==='safrinha'?'2ª cultura (safrinha)':'1ª cultura';
    let s=`<section class="pdf-seq"><h2>${lbl} — ${esc(cultura||'—')}${prod?` · ${num(prod)} sc/ha`:''}</h2>`;
    const info=seqInfo(seq); if(info) s+=`<div class="meta">${info}</div>`;
    // cronograma-resumo: uma linha por operação (ordem, DAE, data prevista, máquina e custos)
    s+=`<table class="pdf-ops pdf-sum"><colgroup><col style="width:5%"><col style="width:29%"><col style="width:7%"><col style="width:11%"><col style="width:20%"><col style="width:9%"><col style="width:9%"><col style="width:10%"></colgroup>
      <thead><tr><th>#</th><th>Operação</th><th class="num">DAE</th><th>Data prev.</th><th>Máquina</th><th class="num">Insumos/ha</th><th class="num">Máq./ha</th><th class="num">Total/ha</th></tr></thead><tbody>`;
    rows.forEach(r=>{ s+=`<tr><td class="num">${r.pos}</td><td><b>${esc(r.op.nome)}</b> <span class="mut2">(${r.items.length} insumo${r.items.length===1?'':'s'})</span></td><td class="num">${r.dae||'—'}</td><td>${r.dp?fmtDataBR(r.dp):'—'}</td><td>${esc(r.conj||'—')}</td><td class="num">${brl(r.sub)}</td><td class="num">${r.mHa?brl(r.mHa):'—'}</td><td class="num"><b>${brl(r.sub+r.mHa)}</b></td></tr>`; });
    s+=`</tbody><tfoot><tr><th colspan="5">Total da ${seq==='safrinha'?'2ª':'1ª'} cultura · ${rows.length} operações · por ha</th><th class="num">${brl(insHa)}</th><th class="num">${brl(maqSeq)}</th><th class="num">${brl(totSeq)}</th></tr>
      <tr class="tot"><th colspan="5">Total na área (${num(area)} ha)</th><th class="num">${brl0(insHa*area)}</th><th class="num">${brl0(maqSeq*area)}</th><th class="num">${brl0(totSeq*area)}</th></tr></tfoot></table>`;
    // detalhe por operação (tabelas com as mesmas larguras em todas as operações)
    rows.forEach(r=>{
      s+=`<div class="op-blk"><h3><span class="op-n">${r.pos}</span> ${esc(r.op.nome)}${r.dae?` <span class="mut2">· ${r.dae} DAE${r.dp?' → '+fmtDataBR(r.dp):''}</span>`:(r.dp?` <span class="mut2">· ${fmtDataBR(r.dp)}</span>`:'')}${r.conj?` <span class="mut2">· 🚜 ${esc(r.conj)}</span>`:''}</h3>
        <table class="pdf-ops pdf-det"><colgroup><col style="width:17%"><col style="width:35%"><col style="width:10%"><col style="width:7%"><col style="width:10%"><col style="width:10%"><col style="width:11%"></colgroup>
        <thead><tr><th>Classe</th><th>Insumo</th><th class="num">Dose/ha</th><th>Un</th><th class="num">Preço</th><th class="num">Custo/ha</th><th class="num">Custo total</th></tr></thead><tbody>`;
      r.items.forEach(it=>{ const p=precoDe(it.produto); s+=`<tr><td>${esc(it.classe||'—')}</td><td>${esc(it.produto)}</td><td class="num">${fmtDose(it.dose)}</td><td>${esc(it.un||'')}</td><td class="num">${p>0?brl(p):'—'}</td><td class="num">${p>0?brl(it.dose*p):'—'}</td><td class="num">${p>0?brl0(it.dose*p*area):'—'}</td></tr>`; });
      s+=`</tbody><tfoot><tr><th colspan="5">Insumos/ha</th><th class="num">${brl(r.sub)}</th><th class="num">${brl0(r.sub*area)}</th></tr>
        ${r.mHa?`<tr><th colspan="5">+ Máquina/ha${r.conj?` (${esc(r.conj)})`:''}</th><th class="num">${brl(r.mHa)}</th><th class="num">${brl0(r.mHa*area)}</th></tr>`:''}
        <tr class="tot"><th colspan="5">Subtotal da operação</th><th class="num">${brl(r.sub+r.mHa)}</th><th class="num">${brl0((r.sub+r.mHa)*area)}</th></tr></tfoot></table></div>`;
    });
    return s+`</section>`;
  };
  ...
  let html=`<div class="pdf-head"><h1>Planejamento do talhão ${esc(t.id)}${t.nome&&t.nome!==t.id?` · ${esc(t.nome)}`:''}</h1>
    ...
    <table class="pdf-ops pdf-kv"><colgroup><col style="width:25%"><col style="width:25%"><col style="width:25%"><col style="width:25%"></colgroup><tbody>
      <tr><td><small>Área</small><b>${num(area)} ha</b></td>
          <td><small>1ª cultura</small><b>${esc(cult1||'—')}</b>${prodvDe(t)?`<br><span class="mut2">${num(prodvDe(t))} sc/ha</span>`:''}</td>
          <td><small>2ª cultura</small><b>${esc(cult2||'—')}</b>${cult2&&prodSafDe(t)?`<br><span class="mut2">${num(prodSafDe(t))} sc/ha</span>`:''}</td>
          <td><small>Custo total do talhão</small><b>${brl0(totHa*area)}</b><br><span class="mut2">${brl(totHa)}/ha</span></td></tr>
      <tr><td><small>Insumos/ha</small><b>${brl(c.ha)}</b></td><td><small>Máquinas/ha</small><b>${brl(maqHa)}</b></td><td><small>Total/ha</small><b>${brl(totHa)}</b></td><td><small>Insumos na área</small><b>${brl0(c.ha*area)}</b></td></tr>
    </tbody></table></div>`;
...
function buildFieldEdits(){
  ...
  // operações NOVAS (além das da planilha) -> cria os blocos lá (processado ANTES dos demais edits do talhão)
  for(const rk in (OV.opExtra||{})){ const cnt=+OV.opExtra[rk]||0; if(!cnt) continue; const p=rk.split('|');
    if(!isBase(p[0])||gone(p[0])) continue; eds.push({type:'addopblock',talhao:p[0],tag:p[1],count:cnt}); }
  ...
  for(const id in OV.talhao){ if(!isBase(id)||gone(id)) continue; const o=OV.talhao[id];
    ...
    if(o.plantio!=null) eds.push({type:'plantio',talhao:id,value:String(o.plantio)});                       // aba do talhão B4
    if(o.plantio_safrinha!=null) eds.push({type:'plantio_safrinha',talhao:id,value:String(o.plantio_safrinha)});   // aba do talhão (resumo safrinha)
    if(o.ciclo!=null) eds.push({type:'ciclo',talhao:id,value:+o.ciclo||0});                               // ciclo (dias) + colheita estimada no resumo
    if(o.ciclo_safrinha!=null) eds.push({type:'ciclo_safrinha',talhao:id,value:+o.ciclo_safrinha||0}); }
  ...
  for(const key in OV.itemAdd){ const i=key.indexOf('|'); if(i<0) continue;
    ...
    if(skip(tid,tag)) continue;
    const baseLen=(planoDe(tid)[seq]||[]).length + ((OV.opExtra&&OV.opExtra[`${tid}|${tag}`])||0);   // + operações novas (bloco criado antes, no mesmo envio)
    if(!(oi<baseLen)) continue;
  ...
  // talhões excluídos no app (que existem na planilha) -> exclui lá (linha + aba)
...
function applyPulledData(d){
  ...
  // preços do módulo Preços também vêm no puxar principal (planilha = fonte da verdade),
...
function reconcileOverrides(){
  ...
  // DAE das operações (compara com o DAP da planilha)
  for(const k in OV.opDae){ const p=k.split('|'); if(p.length<2){ continue; }
    const seq=p[1][0]==='S'?'safrinha':'principal', oi=+p[1].slice(1), op=(planoDe(p[0])[seq]||[])[oi];
    if(op && near(+OV.opDae[k]||0, +op.dap||0)) delete OV.opDae[k]; }
  ...
  Object.keys(OV.talhao).forEach(id=>{ const o=OV.talhao[id], t=DATA.talhoes.find(x=>x.id===id); if(!t) return;
    ...
    const pl=planoDe(id)||{};   // datas de plantio e ciclo vivem na aba do talhão
    if(o.plantio!=null && String(o.plantio)===String(pl.plantio||'').slice(0,10)) delete o.plantio;
    if(o.plantio_safrinha!=null && String(o.plantio_safrinha)===String(pl.plantio_safrinha||'').slice(0,10)) delete o.plantio_safrinha;
    if(o.ciclo!=null && near(o.ciclo, pl.ciclo||0)) delete o.ciclo;
    if(o.ciclo_safrinha!=null && near(o.ciclo_safrinha, pl.ciclo_safrinha||0)) delete o.ciclo_safrinha;
...
async function syncPush(opts){
  ...
  try{
    ...
    const adds=eds.filter(e=>e.type==='additem'), dels=eds.filter(e=>e.type==='delitem'), newT=eds.filter(e=>e.type==='addtalhao'), delT=eds.filter(e=>e.type==='deltalhao'), reo=eds.filter(e=>e.type==='reorderops'), grow=eds.filter(e=>e.type==='addopblock');
    if(res.fail===0 && (adds.length||dels.length||newT.length||delT.length||reo.length||grow.length)){
      grow.forEach(e=>{ if(OV.opExtra) delete OV.opExtra[`${e.talhao}|${e.tag}`]; });   // blocos criados: a planilha agora tem essas operações
...
function alimentarPreview(tipo){
  ...
  const planNorm={};                             // normKey -> nome exato no planejamento (só os REAIS da planilha, não os sintéticos do módulo Preços)
  (DATA.produtos||[]).forEach(pp=>{ if(pp.produto && !pp._precos) planNorm[normKey(pp.produto)]=pp.produto; });
...
async function publicarPlanejamento(){
  ...
  try{
    ...
    // envia TAMBÉM o módulo de Preços (composição) para a planilha, para os outros
    // aparelhos puxarem a lista de preços junto com o resto (sincroniza entre celulares).
    try{ await syncPost(url, JSON.stringify({__precos:PRECOS})); lastPrecosPushSig=JSON.stringify(PRECOS); }catch(e){}
    syncBusy=false; setSyncStatus('ok'); markSynced();
...
// Busca o preço da coluna VALOR (col S do PORTIFÓLIO) do planejamento e traz para
// o Preço à vista do módulo: atualiza os que já existem e acrescenta os que faltam.
async function precosBuscarValor(){
  const url=syncUrl(); if(!url){ toast('Configure a URL de sincronização primeiro'); return; }
  if(syncBusy){ toast('Sincronização ocupada — tente de novo'); return; }
  syncBusy=true; setSyncStatus('busy'); toast('Buscando preços (VALOR) do planejamento…');
  try{
    const d=await syncGet(url,{auto:true});
    const prods=(d&&d.produtos)||[];
    const s=safraAtual(); const have={}; let upd=0, add=0;
    s.itens.forEach((it,idx)=>{ have[normKey(it.produto)]=idx; });
    const byName={}; prods.forEach(pp=>{ if(pp.produto) byName[normKey(pp.produto)]=pp; });
    // atualiza os produtos do módulo com o VALOR do planejamento
    s.itens.forEach(it=>{ const pp=byName[normKey(it.produto)]; if(pp && +pp.preco>0){ it.precoVista=+(+pp.preco).toFixed(2); upd++; } });
    // acrescenta os produtos do planejamento (com preço) que ainda não estão no módulo
    prods.forEach(pp=>{ const k=normKey(pp.produto); if(!k || (k in have)) return; if(+pp.preco>0){
      s.itens.push({empresa:pp.empresa||'', classe:(pp.classe||'').toUpperCase(), produto:pp.produto, precoVista:+(+pp.preco).toFixed(2)}); have[k]=1; add++; } });
    syncBusy=false; setSyncStatus('ok'); markSynced(); savePrecos(); route();
    toast(upd+' preço(s) atualizado(s)'+(add?' · '+add+' novo(s)':'')+' do planejamento');
  }catch(e){ syncBusy=false; setSyncStatus('err'); toast('Falha ao buscar os preços'); }
}
...
function discardEdit(e){
  ...
  switch(e.type){
    case 'dose': delete OV.dose[`${k}|${e.item}`]; break;
    case 'dae': if(OV.opDae) delete OV.opDae[k]; break;
    case 'estoque': delete OV.estoque[e.produto]; break;
    case 'pedido': delete OV.pedido[e.produto]; break;
    case 'itemprod': delete OV.itemProd[e.rk]; break;
    case 'delitem': delete OV.itemRemoved[e.rk]; break;
    case 'additem': { const arr=OV.itemAdd[k]; if(arr){ const i=arr.findIndex(a=>a.produto===e.produto); if(i>=0) arr.splice(i,1); if(!arr.length) delete OV.itemAdd[k]; } break; }
    case 'addopblock': if(OV.opExtra) delete OV.opExtra[`${e.talhao}|${e.tag}`]; break;
    case 'reorderops': if(OV.opPlan) delete OV.opPlan[`${e.talhao}|${seq}`]; if(OV.opExtra) delete OV.opExtra[`${e.talhao}|${e.tag}`];
      if(OV.opDel) for(const x in OV.opDel){ if(x.indexOf(`${e.talhao}|${e.tag}`)===0) delete OV.opDel[x]; } break;
    case 'area': case 'produtividade': case 'empreendimento': case 'emp_safrinha': case 'prod_safrinha':
    case 'plantio': case 'plantio_safrinha': case 'ciclo': case 'ciclo_safrinha': {
      const o=OV.talhao[e.talhao]; if(o){ delete o[e.type]; if(!Object.keys(o).length) delete OV.talhao[e.talhao]; } break; }
...
function pendingPanelHtml(eds){
  ...
  return `<div class="pend-list">${eds.slice(0,lim).map(e=>{ const d=editDesc(e);
    ...
    <p class="mut" style="font-size:11px;margin:10px 0 0">Uma edição só sai desta lista quando a planilha devolve o mesmo valor ao puxar. Se ela continuar aqui depois de sincronizar, pode <b>Descartar</b> (volta a valer o que está na planilha) — ou me mande esta lista.</p></div>`;
}
...
function boot(d){
  DATA=d; PROD={}; d.produtos.forEach(p=>PROD[p.produto]=p);
  loadOverrides(); try{ limparMaqPendentes(); }catch(e){} PRECOS=loadPrecos(); MONIT=loadMonit(); CHUVA=loadChuva(); STAND=loadStand(); RECOM=loadRecom(); COMPRAS=loadCompras(); FLUXO=loadFluxo(); EQUIPE=loadEquipe(); TAREFAS=loadTarefas(); buildMaqIndex(); updateEditBadge();
