...
const APP_VERSION = '2026.07.28-87';   // mostrado no rodapé; ajude a confirmar se a atualização chegou
...
V.compras = function(){
  ...
  <div class="toolbar"><div class="search"><input id="q-compra" placeholder="Buscar produto, classe ou fornecedor…"></div>
    <button class="btn btn-outline btn-sm" id="btn-dem-emp-pdf" title="Relatório de demanda de insumos por empreendimento (PDF)">🖨 Relatório por empreendimento</button>
...
V.cotacao = function(){
  ...
  <div id="cot-groups">${order.map(forn=>{
    ...
    <div class="table-wrap"><table><thead><tr><th>Produto</th><th>Classe</th><th class="num">Qtd</th><th class="num">Saldo</th><th>Un</th><th class="num">Preço ref.</th><th class="num">Valor ref.</th></tr></thead>
    ...
    <tfoot class="tfoot"><tr><td colspan="6">Subtotal ${esc(forn)}</td><td class="num">${brl0(sub)}</td></tr></tfoot></table></div></div>`;
  }).join('')}</div>`;
};
...
function exportDemandaEmpPDF(){
  ...
  const emps=[...new Set(cultivos().map(cv=>cv.emp).filter(e=>e&&e!=='—'&&(!sel.size||sel.has(e))))].sort((a,b)=>a.localeCompare(b,'pt'));
  if(!emps.length){ toast('Sem empreendimentos com demanda'); return; }
  let html=`<div class="pdf-head"><h1>Demanda de insumos por empreendimento — Safra 2026/2027</h1>
    <div class="meta">${emps.length} empreendimento(s) · gerado pelo app Planejamento</div></div>`;
